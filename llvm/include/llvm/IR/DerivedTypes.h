...
class QubitType: public Type {
  ...
  public:
    QubitType(const QubitType &) = delete;
    QubitType &operator=(const QubitType &) = delete;
    
    unsigned long QubitID = 0;

    explicit QubitType(LLVMContext &C, unsigned NumBits) : Type(C, QubitTyID){
    ...
    /// This static method is the primary way of constructing an QubitType.
    /// If an QubitType with the same NumBits value was previously instantiated,
    /// that instance will be returned. Otherwise a new one will be created. Only
    /// one instance with a given NumBits value is ever created.
    /// Get or create an QubitType instance.
    static QubitType *get(LLVMContext &C, unsigned NumBits);

      /// Get the number of bits in this QubitType
    unsigned getBitWidth() const { return getSubclassData(); }
    ...
    static int getNumberOfGate();

    static bool classof(const Type *T) {
      return T->getTypeID() == QubitTyID;
    }
};
