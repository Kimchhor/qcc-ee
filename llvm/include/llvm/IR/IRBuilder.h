...
public:
  ...
  // QCL QCC - gate : define Crate* gate instructions
  //===--------------------------------------------------------------------===//
  // Instruction creation methods: Quantum Instructions
  //===--------------------------------------------------------------------===//

  Value *CreateGateInitX(Value *V, const Twine &Name = ""){
    Instruction *InitX = InitXGateInst::Create(V, Name);
    return Insert(InitX);
  }

  Value *CreateGateInitZ(Value *V, const Twine &Name = ""){
    Instruction *InitZ = InitZGateInst::Create(V, Name);
    return Insert(InitZ);
  }
  
  Value *CreateGateX(Value *V, Value *V1 = nullptr, Value *V2 = nullptr, const Twine &Name = ""){
    Instruction *GateX;

    if (V2)
      GateX = XGateInst::Create(V, V1, V2, Name);
    else if (V1)
      GateX = XGateInst::Create(V,V1, Name);
    else
      GateX = XGateInst::Create(V, Name); 
    return Insert(GateX);
  }

  Value *CreateGateY(Value *V,  Value *V1 = nullptr, Value *V2 = nullptr, const Twine &Name = ""){
    Instruction *GateY;

    if (V2)
      GateY = YGateInst::Create(V, V1, V2, Name);
    else if (V1)
      GateY = YGateInst::Create(V,V1, Name);
    else
      GateY = YGateInst::Create(V, Name); 
    return Insert(GateY);
  }

  Value *CreateGateZ(Value *V,  Value *V1 = nullptr, Value *V2 = nullptr, const Twine &Name = ""){
    
    Instruction *GateZ;
    if (V2)
      GateZ = ZGateInst::Create(V, V1, V2, Name);
    else if (V1)
      GateZ = ZGateInst::Create(V,V1, Name);
    else
      GateZ = ZGateInst::Create(V, Name); 
    return Insert(GateZ);
  }

  Value *CreateGateU(Value *V,  Value *V1 = nullptr, Value *V2 = nullptr, Value *V3 = nullptr, Value *V4 = nullptr, Value *V5 = nullptr, const Twine &Name = ""){
    Instruction *GateU;

    if (V5)
      GateU = UGateInst::Create(V, V1, V2, V3, V4, V5, Name);
    else if (V4) 
      GateU = UGateInst::Create(V, V1, V2, V3, V4, Name);
    else
      GateU = UGateInst::Create(V, V1, V2, V3, Name);
   
    return Insert(GateU);
  }

  Value *CreateGateP(Value *V, Value *V1, Value *V2 = nullptr, const Twine &Name = ""){
    Instruction *GateP;
    GateP = PGateInst::Create(V, V1, Name);
    return Insert(GateP);
  }

  Value *CreateGateS(Value *V, const Twine &Name = ""){
    Instruction *GateS = SGateInst::Create(V, Name);
    return Insert(GateS);
  }

  Value *CreateGateT(Value *V, const Twine &Name = ""){
    Instruction *GateT = TGateInst::Create(V, Name);
    return Insert(GateT);
  }

  Value *CreateGateSdag(Value *V, const Twine &Name = ""){
    Instruction *GateSdag = SdagGateInst::Create(V, Name);
    return Insert(GateSdag);
  }

  Value *CreateGateTdag(Value *V, const Twine &Name = ""){
    Instruction *GateTdag = TdagGateInst::Create(V, Name);
    return Insert(GateTdag);
  }

  Value *CreateGateSx(Value *V, const Twine &Name = ""){
    Instruction *GateSx = SxGateInst::Create(V, Name);
    return Insert(GateSx);
  }

  Value *CreateGateI(Value *V, const Twine &Name = ""){
    Instruction *GateI = IGateInst::Create(V, Name);
    return Insert(GateI);
  }


  Value *CreateGateH(Value *V, Value *V1 = nullptr, const Twine &Name = ""){
    Instruction *GateH;
    if (V1)
      GateH = HGateInst::Create(V,V1, Name);
    else
      GateH = HGateInst::Create(V, Name); 
    return Insert(GateH);
  }

  /// Create new Ctrl that perform on Control qubit
  Value *CreateGateCtrl(Value *V, const Twine &Name = ""){
    Instruction *Gate = CtrlGateInst::Create(V, Name);
    return Insert(Gate);
  }

  Value *CreateGateSwap(Value *V, Value *Ctrl, Value *Ctrl2 = nullptr, const Twine &Name = ""){
    if (Ctrl2){
      Instruction *GateSwap = SwapGateInst::Create(V, Ctrl, Ctrl2, Name);
      return Insert(GateSwap);
    }
    Instruction *GateSwap = SwapGateInst::Create(V, Ctrl, Name);
    return Insert(GateSwap);
  }

  Value *CreateGateMeasureX(Value *V, const Twine &Name = ""){
    Instruction *MeasX = MeasXGateInst::Create(V, Name);
    return Insert(MeasX);
  }

  Value *CreateGateMeasureZ(Value *V, const Twine &Name = ""){
    Instruction *MeasZ = MeasZGateInst::Create(V, Name);
    return Insert(MeasZ);
  }

  Value *CreateGateMeasureY(Value *V, const Twine &Name = ""){
    Instruction *MeasY = MeasYGateInst::Create(V, Name);
    return Insert(MeasY);
  }
