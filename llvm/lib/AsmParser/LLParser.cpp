...
bool LLParser::parseValID(ValID &ID, PerFunctionState *PFS) {
  ...
  // QIR HPCL: Instruction : parse an abstract value
  // Quantum Operations
  case lltok::kw_init_x:
  case lltok::kw_init_z:
  case lltok::kw_x:
  case lltok::kw_y:
  case lltok::kw_z:
  case lltok::kw_h:
  ...
  case lltok::kw_p:
  case lltok::kw_sdg:
  case lltok::kw_measure:
  case lltok::kw_measure_y:
  case lltok::kw_measure_x:
  case lltok::kw_ctrl:
  {
    unsigned Opc = Lex.getUIntVal();
    Constant *Val0;
    ...
    if (parseGlobalTypeAndValue(Val0)){
      return true;
    }

    ID.ConstantVal = ConstantExpr::get(Opc, Val0);
    ID.Kind = ValID::t_Constant;
  ...
  case lltok::kw_swap:{
    ...
    if ( parseGlobalTypeAndValue(Val0) ||
        parseToken(lltok::comma, "expected comma in constantexpr") ||
        parseGlobalTypeAndValue(Val1)) {
      return true;
    }
...
int LLParser::parseInstruction(Instruction *&Inst, BasicBlock *BB,
  ...
  case lltok::kw_init_x: 
    return parseQuantumGate<InitXGateInst>(Inst, PFS);
  case lltok::kw_init_z: 
    return parseQuantumGate<InitZGateInst>(Inst, PFS);
  case lltok::kw_x: 
    return parseQuantumGate<XGateInst>(Inst, PFS);
  case lltok::kw_y: 
    return parseQuantumGate<YGateInst>(Inst, PFS);
  case lltok::kw_z: 
    return parseQuantumGate<ZGateInst>(Inst, PFS);
  case lltok::kw_h: 
    return parseQuantumGate<HGateInst>(Inst, PFS);
  case lltok::kw_i: 
    return parseQuantumGate<IGateInst>(Inst, PFS);
  case lltok::kw_t: 
    return parseQuantumGate<TGateInst>(Inst, PFS);
  case lltok::kw_tdg: 
    return parseQuantumGate<TdagGateInst>(Inst, PFS);
  case lltok::kw_s: 
    return parseQuantumGate<SGateInst>(Inst, PFS);
  case lltok::kw_sdg: 
    return parseQuantumGate<SdagGateInst>(Inst, PFS);
  case lltok::kw_sx:
    return parseQuantumGate<SxGateInst>(Inst, PFS);
  case lltok::kw_p: 
    return parseQuantumGate<PGateInst>(Inst, PFS);
  case lltok::kw_u:
    return parseQuantumGate<UGateInst>(Inst, PFS);
  case lltok::kw_rx: 
    return parseQuantumGate<XGateInst>(Inst, PFS);
  case lltok::kw_ry: 
    return parseQuantumGate<YGateInst>(Inst, PFS);
  case lltok::kw_rz: 
    return parseQuantumGate<ZGateInst>(Inst, PFS);
  case lltok::kw_measure_x: 
    return parseQuantumGate<MeasXGateInst>(Inst, PFS);
  case lltok::kw_measure_y: 
    return parseQuantumGate<MeasYGateInst>(Inst, PFS);
  case lltok::kw_measure: 
    return parseQuantumGate<MeasZGateInst>(Inst, PFS);
  case lltok::kw_swap:
    return parseQuantumGate<SwapGateInst>(Inst, PFS);
  case lltok::kw_ctrl: 
    return parseQuantumGate<CtrlGateInst>(Inst, PFS);
...
/// parseSingleQubitGate
///   ::= 'quantumGate' TypeAndValue ',' TypeAndValue ',' TypeAndValue
template <typename QuantumGate>
bool LLParser::parseSingleQubitGate(Instruction *&Inst, PerFunctionState &PFS){
  
  // LocTy Loc;
  
  // // check if instruction is single qubit gate or multi qubit gate
  // if (Inst->getNumOperands() < 1)
  //   return error(Loc, "invalid quantumGate operands");
  
  // if (Inst->getNumOperands() > 1){
  //   if (Inst->getOperand(1)->getType()->isQubitTy())
  //     return parseMultiQubitGate<QuantumGate>(Inst, PFS);

  //   return parseSingleQubitGate<QuantumGate>(Inst, PFS);
  // }
  return false;
}
...
/// parseQuantumGate
///   ::= '[singleQubitGate]' TypeAndValue ',' TypeAndValue ',' TypeAndValue
template <typename QuantumGate>
bool LLParser::parseQuantumGate(Instruction *&Inst, PerFunctionState &PFS){
  LocTy Loc;
  
  SmallVector<Value *, 0> Ops;
  ...
  do {
    ...
    auto LexKind = Lex.getKind();
    // e.g: %5 = swap q1 %3, %4, %2 (%4 is LocalVarID)
    //                       ~~~
    if (Lex.getKind() == lltok::LocalVarID || Lex.getKind() == lltok::LocalVar) {

      if (parseValue(Type::getQubitTy(Context), Op0, PFS))
        return true;

    } else{

      if (parseTypeAndValue(Op0, PFS))
        return true;
  ...
  switch (Ops.size())
  {
    case 1: Inst = QuantumGate::Create(Ops[0]); break;
    case 2: Inst = QuantumGate::Create(Ops[0], Ops[1]); break;
    case 3: Inst = QuantumGate::Create(Ops[0], Ops[1], Ops[2]); break;
    case 4: Inst = QuantumGate::Create(Ops[0], Ops[1], Ops[2], Ops[3]); break;
    case 5: Inst = QuantumGate::Create(Ops[0], Ops[1], Ops[2], Ops[3], Ops[4]); break;
    case 6: Inst = QuantumGate::Create(Ops[0], Ops[1], Ops[2], Ops[3], Ops[4], Ops[5]); break;
    default: return error(Loc, "invalid quantumGate operands");
  }
...
/// parseMultiQubitGate
///   ::= '[multiQubitGate]' TypeAndValue ',' TypeAndValue ',' TypeAndValue
template <typename QuantumGate>
bool LLParser::parseMultiQubitGate(Instruction *&Inst, PerFunctionState &PFS){
  LocTy Loc;
...
/// parseInitXGate
///   ::= 'initX' TypeAndValue
bool LLParser::parseInitXGate(Instruction *&Inst, PerFunctionState &PFS){
  
  LocTy Loc;
  Value *Op0;

  Inst = InitXGateInst::Create(Op0); 
  return false;
}

/// parseInitZGate
///   ::= 'initZ' TypeAndValue
bool LLParser::parseInitZGate(Instruction *&Inst, PerFunctionState &PFS){
  LocTy Loc;
  Value *Op0;
...
/// parseXGate
///   ::= 'x' TypeAndValue
bool LLParser::parseXGate(Instruction *&Inst, PerFunctionState &PFS){
  ...
  // Debug note
  // Lex.StrVal = variable
  // Lex.CurKind = lltok type

  if (parseTypeAndValue(Op0, PFS)) {
    return true;
  }

  // If parseToken(_) success then it return false
  // Support multiple param of single qubit
  if (parseToken(lltok::comma, "expected ',' after qubit value")) {
      Inst = XGateInst::Create(Op0); 
      return false;
  } 

  // if Op1 is control qubit or if the value
  if (!parseValue(Op0->getType(), Op1, PFS) || !parseTypeAndValue(Op1, PFS)) {
    Inst = XGateInst::Create(Op0, Op1); 
    return false;
  }else{
    return error(Loc, "The parameter is not a qubit or a value");
  }
...
/// parseYGate
///   ::= 'y' TypeAndValue
bool LLParser::parseYGate(Instruction *&Inst, PerFunctionState &PFS){
  ...
  // Support multiple param of single qubit
  if (parseToken(lltok::comma, "expected ',' after qubit value")) {
      Inst = YGateInst::Create(Op0); 
      return false;
  } 

  // if Op1 is control qubit or if the value
  if (!parseValue(Op0->getType(), Op1, PFS) || !parseTypeAndValue(Op1, PFS)) {
    Inst = YGateInst::Create(Op0, Op1); 
    return false;
  }else{
    return error(Loc, "The parameter is not a qubit or a value");
  }
...
/// parseZGate
///   ::= 'z' TypeAndValue
bool LLParser::parseZGate(Instruction *&Inst, PerFunctionState &PFS){
  ...
  // Support multiple param of single qubit
  if (parseToken(lltok::comma, "expected ',' after qubit value")) {
      Inst = ZGateInst::Create(Op0); 
      return false;
  } 

  // if Op1 is control qubit or if the value
  if (!parseValue(Op0->getType(), Op1, PFS) || !parseTypeAndValue(Op1, PFS)) {
    Inst = ZGateInst::Create(Op0, Op1); 
    return false;
  }else{
    return error(Loc, "The parameter is not a qubit or a value");
  }
...
/// parseHGate
///   ::= 'h' TypeAndValue
bool LLParser::parseHGate(Instruction *&Inst, PerFunctionState &PFS){
  ...
  // Support multiple param of single qubit
  if (parseToken(lltok::comma, "expected ',' after qubit value")) {
      Inst = HGateInst::Create(Op0); 
      return false;
  } 

  // if Op1 is control qubit or if the value
  if (!parseValue(Op0->getType(), Op1, PFS) || !parseTypeAndValue(Op1, PFS)) {
    Inst = HGateInst::Create(Op0, Op1); 
    return false;
  }else{
    return error(Loc, "The parameter is not a qubit or a value");
  }
...
/// parseUGate
///   ::= 'u' TypeAndValue
bool LLParser::parseUGate(Instruction *&Inst, PerFunctionState &PFS){
  ...
  if (parseTypeAndValue(Op1, PFS)) {
    return true;
  }

  if (parseToken(lltok::comma, "expected ',' after theta value")) {
      Inst = UGateInst::Create(Op0, Op1);
      return false;
  }

  if (parseTypeAndValue(Op2, PFS)) 
    return error(Loc, "The parameter is not a qubit or a value");
  

  if (parseToken(lltok::comma, "expected ',' after phi value")) {
      Inst = UGateInst::Create(Op0, Op1, Op2);
      return false;
  }

  if (parseTypeAndValue(Op3, PFS))
    return error(Loc, "The parameter is not a qubit or a value");
  

  if (parseToken(lltok::comma, "expected ',' after lambda value")) {
      Inst = UGateInst::Create(Op0, Op1, Op2, Op3);
      return false;
  }

  if (parseTypeAndValue(Op4, PFS))
    return error(Loc, "The parameter is not a qubit or a value");
...
/// parseIGate
///   ::= 'i' TypeAndValue
bool LLParser::parseIGate(Instruction *&Inst, PerFunctionState &PFS){
  
  LocTy Loc;
  Value *Op0;
  // Type *Ty = Type::getQubitPtrTy(Context);

  if (parseTypeAndValue(Op0, PFS)) {
    return true;
  }

  // std::cout << "LLParser::parseIGate: It is : " << getTypeString(Op0->getType()) << std::endl;
  Inst = IGateInst::Create(Op0); 
...
/// parseTGate
///   ::= 't' TypeAndValue
bool LLParser::parseTGate(Instruction *&Inst, PerFunctionState &PFS){
  ...
  // std::cout << "LLParser::parseTGate: It is : " << getTypeString(Op0->getType()) << std::endl;
  Inst = TGateInst::Create(Op0); 
...
/// parseTdagGate
///   ::= 'tdag' TypeAndValue
bool LLParser::parseTdagGate(Instruction *&Inst, PerFunctionState &PFS){
  ...
  // std::cout << "LLParser::parseTdagGate: It is : " << getTypeString(Op0->getType()) << std::endl;
  Inst = TdagGateInst::Create(Op0); 
...
/// parseSxGate
///   ::= 'sx' TypeAndValue
bool LLParser::parseSxGate(Instruction *&Inst, PerFunctionState &PFS){
...
/// parsePGate
///   ::= 'p' TypeAndValue
bool LLParser::parsePGate(Instruction *&Inst, PerFunctionState &PFS){
  ...
  if (parseTypeAndValue(Op0, PFS) || 
      parseToken(lltok::comma, "expected ',' after qubit value") || 
      parseTypeAndValue(Op1, PFS)) {
    return true;
  }

  Inst = PGateInst::Create(Op0, Op1);
...
/// parseSGate
///   ::= 's' TypeAndValue
bool LLParser::parseSGate(Instruction *&Inst, PerFunctionState &PFS){
  ...
  // std::cout << "LLParser::parseSGate: It is : " << getTypeString(Op0->getType()) << std::endl;
  Inst = SGateInst::Create(Op0); 
...
/// parseSdagGate
///   ::= 'sdag' TypeAndValue
bool LLParser::parseSdagGate(Instruction *&Inst, PerFunctionState &PFS){
  ...
  // std::cout << "LLParser::parseSdagGate: It is : " << getTypeString(Op0->getType()) << std::endl;
  Inst = SdagGateInst::Create(Op0); 

  return false;
}


bool LLParser::parseCtrlGate(Instruction *&Inst, PerFunctionState &PFS){
...
/// parseMeasXGate
///   ::= 'meas_x' TypeAndValue
bool LLParser::parseMeasXGate(Instruction *&Inst, PerFunctionState &PFS){
  ...
  // std::cout << "LLParser::parseMeasXGate: It is : " << getTypeString(Op0->getType()) << std::endl;
  Inst = MeasXGateInst::Create(Op0); 
...
/// parseMeasYGate
///   ::= 'meas_y' TypeAndValue
bool LLParser::parseMeasYGate(Instruction *&Inst, PerFunctionState &PFS){
  LocTy Loc;
  Value *Op0;
  ...
  // std::cout << "LLParser::parseMeasYGate: It is : " << getTypeString(Op0->getType()) << std::endl;
  Inst = MeasYGateInst::Create(Op0); 
...
/// parseMeasZGate
///   ::= 'meas_z' TypeAndValue
bool LLParser::parseMeasZGate(Instruction *&Inst, PerFunctionState &PFS){
  ...
  Inst = MeasZGateInst::Create(Op0);
  return false;
}

/// parseSwapGate
///   ::= 'swap' TypeAndValue
bool LLParser::parseSwapGate(Instruction *&Inst, PerFunctionState &PFS){
  ...
  // std::cout << "LLParser::parseSwapGate: It is : " << getTypeString(Op0->getType()) << std::endl;
  Inst = SwapGateInst::Create(Op0, Op1); 
