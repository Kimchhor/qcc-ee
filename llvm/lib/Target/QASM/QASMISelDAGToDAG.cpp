//===-- QASMISelDAGToDAG.cpp - A dag to dag inst selector for QASM --------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines an instruction selector for the QASM target.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "QASM-isel"

namespace {

class QASMDAGToDAGISel : public SelectionDAGISel {
  const QASMSubtarget *Subtarget;

  const QASMTargetMachine &getTargetMachine() const {
    return static_cast<const QASMTargetMachine &>(TM);
  }

  //  const QASMInstrInfo *getInstrInfo() const {
  //    return Subtarget->getInstrInfo();
  //  }

public:
  QASMDAGToDAGISel(QASMTargetMachine &TM, CodeGenOpt::Level OptLevel)
      : SelectionDAGISel(TM, OptLevel) {}

/*
  bool runOnMachineFunction(MachineFunction &MF) override {
    Subtarget = &MF.getSubtarget<QASMSubtarget>();
    return SelectionDAGISel::runOnMachineFunction(MF);
  }
*/
  // Override MachineFunctionPass.
  StringRef getPassName() const override {
    return "QASM DAG->DAG Pattern Instruction Selection";
  }
...
#include "QASMGenDAGISel.inc"
};
} // end anonymous namespace

FunctionPass *llvm::createQASMISelDag(QASMTargetMachine &TM,
                                      CodeGenOpt::Level OptLevel) {
  return new QASMDAGToDAGISel(TM, OptLevel);
}


bool QASMDAGToDAGISel::SelectAddr(SDValue Addr, SDValue &Base, SDValue &Offset) {
...
SDNode *QASMDAGToDAGISel::SelectMeasureGate(SDNode *Node) {
  
  return Node;
}


SDNode *QASMDAGToDAGISel::SelectMoveImmediate(SDNode *N) {
  ...
  MachineSDNode *Move = CurDAG->getMachineNode(QASM::MOVHIi16, N, MVT::i32, SDValue(N, 0),
                                  ConstLo);
  // Select the low part of the immediate move, if needed.
  if (ImmHi) {
    SDValue ConstHi = CurDAG->getTargetConstant(ImmHi >> 16, N, MVT::i32);
    Move = CurDAG->getMachineNode(QASM::MOVHIi16, N, MVT::i32, SDValue(Move, 0),
                                  ConstHi);
  }
...
SDNode *QASMDAGToDAGISel::SelectConditionalBranch(SDNode *N) {
  ...
  SDNode *Compare = CurDAG->getMachineNode(QASMISD::CMP, N, CompareVT, CompareOps);
  ...
  SDValue BranchOps[] = {CCVal, Target, SDValue(Compare, 0),
                         SDValue(Compare, 1)};
  return CurDAG->getMachineNode(QASMISD::BR_CC, N, MVT::Other, BranchOps);
}

void QASMDAGToDAGISel::Select(SDNode *Node) {
