//===-- QASMAsmParser.cpp - Parse QASM assembly to MCInst instructions ----===//
//
...
class QASMOperand : public MCParsedAsmOperand {
  enum OperandKind {
    // KindInvalid,
    OpKind_Token,
    OpKind_Reg,
...
public:
  QASMOperand(OperandKind kind, SMLoc startLoc, SMLoc endLoc)
  ...
  static std::unique_ptr<QASMOperand> createToken(StringRef Str, SMLoc Loc) {
    auto Op = std::make_unique<QASMOperand>(OpKind_Token, Loc, Loc);
    Op->Token.Data = Str.data();
    Op->Token.Length = Str.size();
    return Op;
  }

  static std::unique_ptr<QASMOperand> createReg(unsigned Num, SMLoc StartLoc,
                                                SMLoc EndLoc) {
    auto Op = std::make_unique<QASMOperand>(OpKind_Reg, StartLoc, EndLoc);
    Op->Reg.RegNo = Num;
    return Op;
  }

  static std::unique_ptr<QASMOperand> createImm(const MCExpr *Expr,
                                                SMLoc StartLoc, SMLoc EndLoc) {
    auto Op = std::make_unique<QASMOperand>(OpKind_Imm, StartLoc, EndLoc);
    Op->Imm = Expr;
    return Op;
  }
...
class QASMAsmParser : public MCTargetAsmParser {
// Auto-generated instruction matching functions
#define GET_ASSEMBLER_HEADER
#include "QASMGenAsmMatcher.inc"
...
public:
  QASMAsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser,
...
bool QASMAsmParser::ParseDirective(AsmToken DirectiveID) { return true; }

bool QASMAsmParser::ParseInstruction(ParseInstructionInfo &Info, StringRef Name,
                                     SMLoc NameLoc, OperandVector &Operands) {
  // First operand in MCInst is instruction mnemonic.
  Operands.push_back(QASMOperand::createToken(Name, NameLoc));
...
bool QASMAsmParser::parseOperand(OperandVector &Operands, StringRef Mnemonic) {
  ...
  if (Lexer.is(AsmToken::Percent)) {
    ...
    if (ParseRegister(RegNo, StartLoc, EndLoc))
      return true;
    Operands.push_back(QASMOperand::createReg(RegNo, StartLoc, EndLoc));
    return false;
  }
  ...
  if (Lexer.is(AsmToken::Integer)) {
    ...
    Operands.push_back(QASMOperand::createImm(Expr, StartLoc, EndLoc));
    return false;
  }
...
OperandMatchResultTy QASMAsmParser::parseImmWO(OperandVector &Operands) {
...
bool QASMAsmParser::ParseRegister(unsigned &RegNo, SMLoc &StartLoc,
...
OperandMatchResultTy QASMAsmParser::tryParseRegister(unsigned &RegNo,
                                                     SMLoc &StartLoc,
                                                     SMLoc &EndLoc) {
  bool Result =
...
bool QASMAsmParser::MatchAndEmitInstruction(SMLoc IdLoc, unsigned &Opcode,
  ...
  case Match_InvalidOperand: {
    ...
    if (ErrorInfo != ~0U) {
      ...
      // TODO
      // ErrorLoc = ((QASMOperand &)*Operands[ErrorInfo]).getStartLoc();
      if (ErrorLoc == SMLoc())
        ErrorLoc = IdLoc;
    }
  ...
  case Match_MnemonicFail: {
    ...
    QASMOperand &Op = static_cast<QASMOperand &>(*Operands[0]);
    std::string Suggestion = QASMMnemonicSpellCheck(Op.getToken(), FBS, 0);
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeQASMAsmParser() {
  RegisterMCAsmParser<QASMAsmParser> x(getTheQASMTarget());
}
