//===-- QASMMachineFuctionInfo.h - QASM machine function info -----*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file declares QASM-specific per-machine-function information.
//
//===----------------------------------------------------------------------===//

#ifndef LLVM_QASM_MACHINE_FUNCTION_INFO_H
#define LLVM_QASM_MACHINE_FUNCTION_INFO_H
...
enum DataRegisterType{
  IDS = 0,
  UDS = 1,
  RDS = 2, // Rotation float
};
/// Contains QASM-specific information for each MachineFunction.
class QASMMachineFunctionInfo : public MachineFunctionInfo {
  ...
  /// RData Segment register size
  unsigned RDSRegisterSize = 0;
...
public:
  QASMMachineFunctionInfo()
      : HasSpills(false), HasAllocas(false), HasStackArgs(false),
        IsInterruptHandler(false), IsSignalHandler(false),
        CalleeSavedFrameSize(0), VarArgsFrameIndex(0), IDSRegisterSize(0), UDSRegisterSize(0), RDSRegisterSize(0) {}

  explicit QASMMachineFunctionInfo(MachineFunction &MF)
      : HasSpills(false), HasAllocas(false), HasStackArgs(false),
  ...
  unsigned getRDSRegisterSize() const {return RDSRegisterSize;}
  void setRDSRegisterSize(unsigned size) { RDSRegisterSize = size; }
  ...
  bool isRDSRegister(unsigned FrameIndex) const { return dataRegisterType[FrameIndex] == RDS;}
...
#endif // LLVM_QASM_MACHINE_FUNCTION_INFO_H
