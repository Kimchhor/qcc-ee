//===-- QISAAsmParser.cpp - Parse QISA assembly to MCInst instructions ----===//
//
...
class QISAOperand : public MCParsedAsmOperand {
  enum OperandKind {
    // KindInvalid,
    OpKind_Token,
    OpKind_Reg,
...
public:
  QISAOperand(OperandKind kind, SMLoc startLoc, SMLoc endLoc)
  ...
  static std::unique_ptr<QISAOperand> createToken(StringRef Str, SMLoc Loc) {
    auto Op = std::make_unique<QISAOperand>(OpKind_Token, Loc, Loc);
    Op->Token.Data = Str.data();
    Op->Token.Length = Str.size();
    return Op;
  }

  static std::unique_ptr<QISAOperand> createReg(unsigned Num, SMLoc StartLoc,
                                                SMLoc EndLoc) {
    auto Op = std::make_unique<QISAOperand>(OpKind_Reg, StartLoc, EndLoc);
    Op->Reg.RegNo = Num;
    return Op;
  }

  static std::unique_ptr<QISAOperand> createImm(const MCExpr *Expr,
                                                SMLoc StartLoc, SMLoc EndLoc) {
    auto Op = std::make_unique<QISAOperand>(OpKind_Imm, StartLoc, EndLoc);
    Op->Imm = Expr;
    return Op;
  }
...
class QISAAsmParser : public MCTargetAsmParser {
// Auto-generated instruction matching functions
#define GET_ASSEMBLER_HEADER
#include "QISAGenAsmMatcher.inc"
...
public:
  QISAAsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser,
...
bool QISAAsmParser::ParseDirective(AsmToken DirectiveID) { return true; }

bool QISAAsmParser::ParseInstruction(ParseInstructionInfo &Info, StringRef Name,
                                     SMLoc NameLoc, OperandVector &Operands) {
  // First operand in MCInst is instruction mnemonic.
  Operands.push_back(QISAOperand::createToken(Name, NameLoc));
...
bool QISAAsmParser::parseOperand(OperandVector &Operands, StringRef Mnemonic) {
  ...
  if (Lexer.is(AsmToken::Percent)) {
    ...
    if (ParseRegister(RegNo, StartLoc, EndLoc))
      return true;
    Operands.push_back(QISAOperand::createReg(RegNo, StartLoc, EndLoc));
    return false;
  }
  ...
  if (Lexer.is(AsmToken::Integer)) {
    ...
    Operands.push_back(QISAOperand::createImm(Expr, StartLoc, EndLoc));
    return false;
  }
...
OperandMatchResultTy QISAAsmParser::parseImmWO(OperandVector &Operands) {
...
bool QISAAsmParser::ParseRegister(unsigned &RegNo, SMLoc &StartLoc,
...
OperandMatchResultTy QISAAsmParser::tryParseRegister(unsigned &RegNo,
                                                     SMLoc &StartLoc,
                                                     SMLoc &EndLoc) {
  bool Result =
...
bool QISAAsmParser::MatchAndEmitInstruction(SMLoc IdLoc, unsigned &Opcode,
  ...
  case Match_InvalidOperand: {
    ...
    if (ErrorInfo != ~0U) {
      ...
      // TODO
      // ErrorLoc = ((QISAOperand &)*Operands[ErrorInfo]).getStartLoc();
      if (ErrorLoc == SMLoc())
        ErrorLoc = IdLoc;
    }
  ...
  case Match_MnemonicFail: {
    ...
    QISAOperand &Op = static_cast<QISAOperand &>(*Operands[0]);
    std::string Suggestion = QISAMnemonicSpellCheck(Op.getToken(), FBS, 0);
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeQISAAsmParser() {
  RegisterMCAsmParser<QISAAsmParser> x(getTheQISATarget());
}
