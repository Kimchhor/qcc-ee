...
// TODO QIR: Implement Codegen IR

bool IRTranslator::translateInitXGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateInitZGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateXGate(const User &U, MachineIRBuilder &MIRBuilder){

  // Register Res = getOrCreateVReg(U);
  // Register Val = getOrCreateVReg(*U.getOperand(0));

  // MIRBuilder.buildXGate(Res, Val);
  return true;
}
bool IRTranslator::translateYGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateZGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateHGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateIGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateTGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateTdagGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateSGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateSdagGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateSxGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translatePGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateUGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateRxGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateRyGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateRzGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateCX0Gate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateCX1Gate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateCtrlGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateMeasXGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateMeasYGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
bool IRTranslator::translateMeasZGate(const User &U, MachineIRBuilder &MIRBuilder){
    // Get or create a virtual register for each value.
  // Unless the value is a Constant => loadimm cst?
  // or inline constant each time?
  // Creation of a virtual register needs to have a size.
  ...
  // MIRBuilder.buildInstr(TargetOpcode::MEAS_Z_GATE, {Res}, {Op0, Op1}, Flags);

  // NOTE: I try to add the target opcode in TargetOpcodes.def too.
  // However, it still not working. So just return true.
  return true;
}
bool IRTranslator::translateSwapGate(const User &U, MachineIRBuilder &MIRBuilder){
  return true;
}
