...
void DAGTypeLegalizer::PromoteIntegerResult(SDNode *N, unsigned ResNo) {
  ...
  // QCL QCC - gate : promote int in measure gate
  case ISD::MEAS_Z_GATE:
  case ISD::MEAS_X_GATE:
  case ISD::MEAS_Y_GATE:
      Res = PromoteIntRes_MEASURE_GATE(N); break;
...
// QCL QCC - backend : TODO - implement promote int
SDValue DAGTypeLegalizer::PromoteIntRes_MEASURE_GATE(SDNode *N){

  // EVT ResVT = TLI.getTypeToTransformTo(*DAG.getContext(), N->getValueType(1));
  ...
  // Legalize the chain result - switch anything that used the old chain to
  // use the new one.
  SDValue Res = DAG.getNode(ISD::MEAS_Z_GATE, dl, NOutVT, GetPromotedInteger(InOp));
  ReplaceValueWith(SDValue(N, 0), Res.getValue(0));
  return Res;
