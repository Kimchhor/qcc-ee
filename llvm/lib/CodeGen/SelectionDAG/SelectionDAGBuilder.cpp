...
// QIR HPCL: Instruction : define vist* fucntion for SelectionDAG
// This function takes one or two operands of the binary operator from IR and 
// stores them into SDValue type. Then it invokes the DAG.getNode() function 
// with opcode of the qunnatum operator.
void SelectionDAGBuilder::visitInitXGate(const User &I) {
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();
  SDValue InQubit = getValue(I.getOperand(0));

  // set DAG
  setValue(&I, DAG.getNode(ISD::INIT_X_GATE, getCurSDLoc(), TLI.getValueType(DAG.getDataLayout(), I.getType()), InQubit));

}
void SelectionDAGBuilder::visitInitZGate(const User &I) {
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();
  SDValue InQubit = getValue(I.getOperand(0));

  // set DAG
  setValue(&I, DAG.getNode(ISD::INIT_Z_GATE, getCurSDLoc(), TLI.getValueType(DAG.getDataLayout(), I.getType()), InQubit));

}

void SelectionDAGBuilder::visitXGate(const User &I) {
  
  // Somehow we need to set a flag to share to IR optimizer
  SDNodeFlags Flags;
  ...
  SDValue GateNodeValue = DAG.getNode(ISD::X_GATE, getCurSDLoc(),Ops[0].getValueType(), makeArrayRef(Ops.data(), NumOps), Flags);

  setValue(&I, GateNodeValue);
}

void SelectionDAGBuilder::visitYGate(const User &I) {
  ...
  SDValue GateNodeValue = DAG.getNode(ISD::Y_GATE, getCurSDLoc(),Ops[0].getValueType(), makeArrayRef(Ops.data(), NumOps), Flags);

  setValue(&I, GateNodeValue);
}

void SelectionDAGBuilder::visitZGate(const User &I) {
  ...
  SDValue GateNodeValue = DAG.getNode(ISD::Z_GATE, getCurSDLoc(),Ops[0].getValueType(), makeArrayRef(Ops.data(), NumOps), Flags);

  setValue(&I, GateNodeValue);
}

void SelectionDAGBuilder::visitPGate(const User &I) {
  SDValue Op1 = getValue(I.getOperand(0));
  SDValue Op2 = getValue(I.getOperand(1));

  SDNodeFlags Flags;
  
  setValue(&I, DAG.getNode(ISD::P_GATE, getCurSDLoc(),Op1.getValueType(), Op1, Op2, Flags));
}


void SelectionDAGBuilder::visitHGate(const User &I) {
  // Somehow we need to set a flag to share to IR optimizer
  SDNodeFlags Flags;
  ...
  SDValue GateNodeValue = DAG.getNode(ISD::H_GATE, getCurSDLoc(),Ops[0].getValueType(), makeArrayRef(Ops.data(), NumOps), Flags);

  setValue(&I, GateNodeValue);

}

void SelectionDAGBuilder::visitIGate(const User &I) {
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();
  SDValue InQubit = getValue(I.getOperand(0));

  // set DAG
  setValue(&I, DAG.getNode(ISD::I_GATE, getCurSDLoc(), TLI.getValueType(DAG.getDataLayout(), I.getType()), InQubit));

}

void SelectionDAGBuilder::visitUGate(const User &I) {
  // Somehow we need to set a flag to share to IR optimizer
  SDNodeFlags Flags;
  ...
  SDValue GateNodeValue = DAG.getNode(ISD::U_GATE, getCurSDLoc(),Ops[0].getValueType(), makeArrayRef(Ops.data(), NumOps), Flags);

  setValue(&I, GateNodeValue);
}

void SelectionDAGBuilder::visitTGate(const User &I) {
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();
  SDValue InQubit = getValue(I.getOperand(0));

  // set DAG
  setValue(&I, DAG.getNode(ISD::T_GATE, getCurSDLoc(), TLI.getValueType(DAG.getDataLayout(), I.getType()), InQubit));

}

void SelectionDAGBuilder::visitTdagGate(const User &I) {
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();
  SDValue InQubit = getValue(I.getOperand(0));

  // set DAG
  setValue(&I, DAG.getNode(ISD::TDAG_GATE, getCurSDLoc(), TLI.getValueType(DAG.getDataLayout(), I.getType()), InQubit));

}

void SelectionDAGBuilder::visitSGate(const User &I) {
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();
  SDValue InQubit = getValue(I.getOperand(0));

  // set DAG
  setValue(&I, DAG.getNode(ISD::S_GATE, getCurSDLoc(), TLI.getValueType(DAG.getDataLayout(), I.getType()), InQubit));

}

void SelectionDAGBuilder::visitSdagGate(const User &I) {
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();
  SDValue InQubit = getValue(I.getOperand(0));

  // set DAG
  setValue(&I, DAG.getNode(ISD::SDAG_GATE, getCurSDLoc(), TLI.getValueType(DAG.getDataLayout(), I.getType()), InQubit));

}

void SelectionDAGBuilder::visitSxGate(const User &I) {
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();
  SDValue InQubit = getValue(I.getOperand(0));

  // set DAG
  setValue(&I, DAG.getNode(ISD::SX_GATE, getCurSDLoc(), TLI.getValueType(DAG.getDataLayout(), I.getType()), InQubit));

}

void SelectionDAGBuilder::visitRxGate(const User &I) {
  // Somehow we need to set a flag to share to IR optimizer
  SDNodeFlags Flags;
  ...
  SDValue GateNodeValue = DAG.getNode(ISD::RX_GATE, getCurSDLoc(),Ops[0].getValueType(), makeArrayRef(Ops.data(), NumOps), Flags);

  setValue(&I, GateNodeValue);
}

void SelectionDAGBuilder::visitRyGate(const User &I) {
  // Somehow we need to set a flag to share to IR optimizer
  SDNodeFlags Flags;
  ...
  SDValue GateNodeValue = DAG.getNode(ISD::RY_GATE, getCurSDLoc(),Ops[0].getValueType(), makeArrayRef(Ops.data(), NumOps), Flags);

  setValue(&I, GateNodeValue);

}

void SelectionDAGBuilder::visitRzGate(const User &I) {
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();
  SDValue InQubit = getValue(I.getOperand(0));
  SDValue InRotate = getValue(I.getOperand(1));
  // set DAG
  setValue(&I, DAG.getNode(ISD::RZ_GATE, getCurSDLoc(), TLI.getValueType(DAG.getDataLayout(), I.getType()), InQubit, InRotate));

}

void SelectionDAGBuilder::visitCX0Gate(const User &I) {
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();
  SDValue InQubit = getValue(I.getOperand(0));
  SDValue InQID = getValue(I.getOperand(1));
  // set DAG
  setValue(&I, DAG.getNode(ISD::CX0_GATE, getCurSDLoc(), TLI.getValueType(DAG.getDataLayout(), I.getType()), InQubit, InQID));

}

void SelectionDAGBuilder::visitCX1Gate(const User &I) {
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();
  SDValue InQubit = getValue(I.getOperand(0));
  SDValue InQID = getValue(I.getOperand(1));
  // set DAG
  setValue(&I, DAG.getNode(ISD::CX1_GATE, getCurSDLoc(), InQubit.getValueType(), InQubit, InQID));
}

void SelectionDAGBuilder::visitCtrlGate(const User &I) {
  SDValue InQubit = getValue(I.getOperand(0));
  // set DAG
  setValue(&I, DAG.getNode(ISD::CTRL_GATE, getCurSDLoc(), InQubit.getValueType(), InQubit));
}

void SelectionDAGBuilder::visitMeasXGate(const User &I) {
  SDValue InQubit = getValue(I.getOperand(0));

  SDValue MeasX = DAG.getNode(ISD::MEAS_X_GATE, getCurSDLoc(), MVT::i32, InQubit);

  setValue(&I, MeasX);
}

void SelectionDAGBuilder::visitMeasYGate(const User &I) {
  SDValue InQubit = getValue(I.getOperand(0));
  SDValue MeasY = DAG.getNode(ISD::MEAS_Y_GATE, getCurSDLoc(), MVT::i32, InQubit);

  setValue(&I, MeasY);
}

void SelectionDAGBuilder::visitMeasZGate(const User &I) {
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();
  SDValue Op1 = getValue(I.getOperand(0));

  SDValue MeasZ = DAG.getNode(ISD::MEAS_Z_GATE, getCurSDLoc(), MVT::i32, Op1);

  setValue(&I, MeasZ);
}

void SelectionDAGBuilder::visitSwapGate(const User &I) {
  // Somehow we need to set a flag to share to IR optimizer
  SDNodeFlags Flags;
  ...
  SDValue GateNodeValue = DAG.getNode(ISD::SWAP_GATE, getCurSDLoc(),Ops[0].getValueType(), makeArrayRef(Ops.data(), NumOps), Flags);
