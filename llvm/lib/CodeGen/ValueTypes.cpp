...
std::string EVT::getEVTString() const {
  ...
  // QCL QCC - backend : get EVT string
  case MVT::qubit:     return "qubit";
  }
}
...
Type *EVT::getTypeForEVT(LLVMContext &Context) const {
  ...
  default:
    assert(isExtended() && "Type is not extended!");
    return LLVMTy;
  // QCL QCC - backend : define type for EVT
  case MVT::qubit:   return Type::getQubitTy(Context);
...
MVT MVT::getVT(Type *Ty, bool HandleUnknown){
  ...
  // QCL QCC - backend : Define machine value type TODO i32 change to qubit
  case Type::QubitTyID:     return MVT(MVT::qubit);
