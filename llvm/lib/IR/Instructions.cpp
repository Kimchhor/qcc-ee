...
template <typename T>
static Instruction* insertGateBefore(Value* q1, Instruction* prevInst, unsigned updateOp) {
    Instruction *NewInst = T::Create(q1);
    NewInst->insertBefore(prevInst);
    prevInst->setOperand(updateOp, NewInst);
    return NewInst;
}


template <typename T>
static Instruction* insertGateAfter(Value* q1, Instruction* prevInst, unsigned updateOp) {
    // TODO: Test this function
    Instruction *NewInst = T::Create(prevInst);
...
template <typename T>
static Instruction* insertTwoQuibtGateBefore(Value* q1, Value* q2, Instruction* prevInst, unsigned updateOp) {
    Instruction *NewInst = T::Create(q1, q2);
    NewInst->insertBefore(prevInst);
    prevInst->setOperand(updateOp, NewInst);
    return NewInst;
}
...
template <typename T>
static std::function<SmallVector<Instruction*>(SmallVector<Instruction*>, Instruction*)> applyGate(int index){
  using ListInst = SmallVector<Instruction*>;
  return [index](ListInst params, Instruction* prevInst) -> ListInst {
        params[index] = insertGateBefore<T>(params[index], prevInst, index);
        return params; 
      };
}

template <typename T>
static std::function<SmallVector<Instruction*>(SmallVector<Instruction*>, Instruction*)> applyTwoQubitGate(int ctrlIdx, int tarIdx){
  using ListInst = SmallVector<Instruction*>;
  return [ctrlIdx, tarIdx](ListInst params, Instruction* prevInst) -> ListInst {
        params[tarIdx] = insertTwoQuibtGateBefore<T>(params[ctrlIdx], params[tarIdx], prevInst, tarIdx);
        return params; 
      };
}

template <typename T>
static std::pair<Instruction*, Instruction*> insertTwoQubitGateBefore(Value* q1, Value* q2, Instruction* prevInst, unsigned updateOp, unsigned updateOp2) {

    Instruction *Target = T::Create(q1, q2);
    Target->insertBefore(prevInst);
    prevInst->setOperand(updateOp2, Target);

    Instruction *Ctrl = insertGateBefore<CtrlGateInst>(q1, Target, 0);

    prevInst->setOperand(updateOp, Ctrl);
    
    return std::make_pair(Ctrl, Target);
}

//===----------------------------------------------------------------------===//
//                      InitXGateInst Implementation
//===----------------------------------------------------------------------===//


InitXGateInst::InitXGateInst(Value *Qubit, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), InitXGate, OperandTraits<InitXGateInst>::op_begin(this), 1, InsertBef){
   Op<0>() = Qubit;

  setName(Name);
}

InitXGateInst::InitXGateInst( Value *Qubit, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), InitZGate, OperandTraits<InitXGateInst>::op_begin(this), 1, InsertAtEnd){
...
InitXGateInst *InitXGateInst::cloneImpl() const {
  return new InitXGateInst(getOperand(0));
}

//===----------------------------------------------------------------------===//
//                      InitZGateInst Implementation
//===----------------------------------------------------------------------===//


InitZGateInst::InitZGateInst(Value *Qubit, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), InitZGate, OperandTraits<InitZGateInst>::op_begin(this), 1, InsertBef){
   Op<0>() = Qubit;

  setName(Name);
}

InitZGateInst::InitZGateInst( Value *Qubit, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), InitZGate, OperandTraits<InitZGateInst>::op_begin(this), 1, InsertAtEnd){
...
InitZGateInst *InitZGateInst::cloneImpl() const {
  return new InitZGateInst(getOperand(0));
}


//===----------------------------------------------------------------------===//
//                      XGateInst Implementation
//===----------------------------------------------------------------------===//


XGateInst::XGateInst(Value *Qubit, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), XGate, OperandTraits<XGateInst>::op_begin(this), 1, InsertBef){
   Op<0>() = Qubit;

  setName(Name);
}

XGateInst::XGateInst( Value *Qubit, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), XGate, OperandTraits<XGateInst>::op_begin(this), 1, InsertAtEnd){
...
XGateInst::XGateInst(Value *Qubit, Value *Qubit1, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), XGate, OperandTraits<XGateInst>::op_begin(this), 2, InsertBef){
   Op<0>() = Qubit;
   Op<1>() = Qubit1;
...
XGateInst::XGateInst( Value *Qubit,Value *Qubit1, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), XGate, OperandTraits<XGateInst>::op_begin(this), 2, InsertAtEnd){
...
XGateInst::XGateInst(Value *Qubit, Value *Qubit1, Value *Qubit2, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), XGate, OperandTraits<XGateInst>::op_begin(this), 3, InsertBef){
  Op<0>() = Qubit;
  Op<1>() = Qubit1;
  Op<2>() = Qubit2;
  setName(Name);
  
  // check if Qubit, Qubit1, Qubit2 are QubitTy
  if (Qubit->getType()->isQubitTy() && 
      Qubit1->getType()->isQubitTy() && 
      Qubit2->getType()->isQubitTy()) {

    this->shouldDecompose = true;
    ...
    auto H    = applyGate<HGateInst>;
    auto Ctrl = applyGate<CtrlGateInst>;
    auto Tdg  = applyGate<TdagGateInst>;
    auto T    = applyGate<TGateInst>;
    auto X2   = applyTwoQubitGate<XGateInst>;
...
XGateInst::XGateInst( Value *Qubit,Value *Qubit1, Value *Qubit2, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), XGate, OperandTraits<XGateInst>::op_begin(this), 3, InsertAtEnd){
...
XGateInst *XGateInst::cloneImpl() const {
  return new XGateInst(getOperand(0), getOperand(1));
}

//===----------------------------------------------------------------------===//
//                      YGateInst Implementation
//===----------------------------------------------------------------------===//


YGateInst::YGateInst(Value *Qubit, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), YGate, OperandTraits<YGateInst>::op_begin(this), 1, InsertBef){
   Op<0>() = Qubit;

  setName(Name);
}

YGateInst::YGateInst( Value *Qubit, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), YGate, OperandTraits<YGateInst>::op_begin(this), 1, InsertAtEnd){
...
YGateInst::YGateInst(Value *Qubit, Value *Qubit1, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), YGate, OperandTraits<YGateInst>::op_begin(this), 2, InsertBef){
   Op<0>() = Qubit;
   Op<1>() = Qubit1;
...
YGateInst::YGateInst( Value *Qubit,Value *Qubit1, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), YGate, OperandTraits<YGateInst>::op_begin(this), 2, InsertAtEnd){
...
YGateInst::YGateInst(Value *Qubit, Value *Qubit1, Value *Qubit2, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), YGate, OperandTraits<YGateInst>::op_begin(this), 3, InsertBef){
  Op<0>() = Qubit;
  Op<1>() = Qubit1;
  Op<2>() = Qubit2;
...
YGateInst::YGateInst( Value *Qubit,Value *Qubit1, Value *Qubit2, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), YGate, OperandTraits<YGateInst>::op_begin(this), 3, InsertAtEnd){
...
YGateInst *YGateInst::cloneImpl() const {
  return new YGateInst(getOperand(0), getOperand(1));
}


//===----------------------------------------------------------------------===//
//                      PGateInst Implementation
//===----------------------------------------------------------------------===//


PGateInst::PGateInst(Value *Qubit, Value *Qubit1, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), PGate, OperandTraits<PGateInst>::op_begin(this), 2, InsertBef){
   Op<0>() = Qubit;
   Op<1>() = Qubit1;
...
PGateInst::PGateInst( Value *Qubit,Value *Qubit1, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), PGate, OperandTraits<PGateInst>::op_begin(this), 2, InsertAtEnd){
...
PGateInst::PGateInst(Value *Qubit, Value *Qubit1, Value *Qubit2, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), PGate, OperandTraits<PGateInst>::op_begin(this), 3, InsertBef){
   Op<0>() = Qubit;
   Op<1>() = Qubit1;
  Op<2>() = Qubit2;
  setName(Name);
...
PGateInst::PGateInst( Value *Qubit,Value *Qubit1, Value *Qubit2, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), PGate, OperandTraits<PGateInst>::op_begin(this), 3, InsertAtEnd){
...
PGateInst *PGateInst::cloneImpl() const {
  return new PGateInst(getOperand(0), getOperand(1));
}

//===----------------------------------------------------------------------===//
//                      ZGateInst Implementation
//===----------------------------------------------------------------------===//


ZGateInst::ZGateInst(Value *Qubit, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), ZGate, OperandTraits<ZGateInst>::op_begin(this), 1, InsertBef){
   Op<0>() = Qubit;

  setName(Name);
}

ZGateInst::ZGateInst( Value *Qubit, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), ZGate, OperandTraits<ZGateInst>::op_begin(this), 1, InsertAtEnd){
...
ZGateInst::ZGateInst(Value *Qubit, Value *Qubit1, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), ZGate, OperandTraits<ZGateInst>::op_begin(this), 2, InsertBef){
   Op<0>() = Qubit;
   Op<1>() = Qubit1;
...
ZGateInst::ZGateInst( Value *Qubit,Value *Qubit1, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), ZGate, OperandTraits<ZGateInst>::op_begin(this), 2, InsertAtEnd){
...
ZGateInst::ZGateInst(Value *Qubit, Value *Qubit1, Value *Qubit2, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), ZGate, OperandTraits<ZGateInst>::op_begin(this), 3, InsertBef){
  Op<0>() = Qubit;
  Op<1>() = Qubit1;
  Op<2>() = Qubit2;
...
ZGateInst::ZGateInst( Value *Qubit,Value *Qubit1, Value *Qubit2, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), ZGate, OperandTraits<ZGateInst>::op_begin(this), 3, InsertAtEnd){
...
ZGateInst *ZGateInst::cloneImpl() const {
  return new ZGateInst(getOperand(0), getOperand(1));
}


//===----------------------------------------------------------------------===//
//                      HGateInst Implementation
//===----------------------------------------------------------------------===//


HGateInst::HGateInst(Value *Qubit, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), HGate, OperandTraits<HGateInst>::op_begin(this), 1, InsertBef){
   Op<0>() = Qubit;

  setName(Name);
}

HGateInst::HGateInst( Value *Qubit, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), HGate, OperandTraits<HGateInst>::op_begin(this), 1, InsertAtEnd){
...
HGateInst::HGateInst(Value *Qubit, Value *Qubit1, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), HGate, OperandTraits<HGateInst>::op_begin(this), 2, InsertBef){
   Op<0>() = Qubit;
   Op<1>() = Qubit1;
...
HGateInst::HGateInst( Value *Qubit,Value *Qubit1, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), HGate, OperandTraits<HGateInst>::op_begin(this), 2, InsertAtEnd){
...
HGateInst *HGateInst::cloneImpl() const {
  return new HGateInst(getOperand(0));
}

//===----------------------------------------------------------------------===//
//                      UGateInst Implementation
//===----------------------------------------------------------------------===//


UGateInst::UGateInst( Value *Qubit, Value *Theta, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), UGate, OperandTraits<UGateInst>::op_begin(this), 2, InsertBef){
   Op<0>() = Qubit;
   Op<1>() = Theta;
  setName(Name);
}

UGateInst::UGateInst( Value *Qubit, Value *Theta,  const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), UGate, OperandTraits<UGateInst>::op_begin(this), 2, InsertAtEnd){
  Op<0>() = Qubit;
  Op<1>() = Theta;
  setName(NameStr);
}

UGateInst::UGateInst( Value *Qubit, Value *Theta, Value *Phi, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), UGate, OperandTraits<UGateInst>::op_begin(this), 3, InsertBef){
   Op<0>() = Qubit;
   Op<1>() = Theta;
   Op<2>() = Phi;
  setName(Name);
...
UGateInst::UGateInst( Value *Qubit, Value *Theta, Value *Phi, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), UGate, OperandTraits<UGateInst>::op_begin(this), 3, InsertAtEnd){
  Op<0>() = Qubit;
  Op<1>() = Theta;
  Op<2>() = Phi;
  setName(NameStr);
...
UGateInst::UGateInst( Value *Qubit, Value *Theta, Value *Phi, Value *Lambda, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), UGate, OperandTraits<UGateInst>::op_begin(this), 4, InsertBef){
   Op<0>() = Qubit;
   Op<1>() = Theta;
   Op<2>() = Phi;
   Op<3>() = Lambda;
...
UGateInst::UGateInst( Value *Qubit, Value *Theta, Value *Phi, Value *Lambda, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), UGate, OperandTraits<UGateInst>::op_begin(this), 4, InsertAtEnd){
  Op<0>() = Qubit;
  Op<1>() = Theta;
  Op<2>() = Phi;
  Op<3>() = Lambda;
...
UGateInst::UGateInst( Value *Qubit, Value *Theta, Value *Phi, Value *Lambda, Value* Qubit1, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), UGate, OperandTraits<UGateInst>::op_begin(this), 5, InsertBef){
  Op<0>() = Qubit;
  Op<1>() = Theta;
  Op<2>() = Phi;
  Op<3>() = Lambda;
...
UGateInst::UGateInst( Value *Qubit, Value *Theta, Value *Phi, Value *Lambda, Value* Qubit1, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), UGate, OperandTraits<UGateInst>::op_begin(this), 5, InsertAtEnd){
  Op<0>() = Qubit;
  Op<1>() = Theta;
  Op<2>() = Phi;
  Op<3>() = Lambda;
...
UGateInst::UGateInst( Value *Qubit, Value *Theta, Value *Phi, Value *Lambda, Value* Qubit1, Value* Qubit2, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), UGate, OperandTraits<UGateInst>::op_begin(this), 6, InsertBef){
  Op<0>() = Qubit;
  Op<1>() = Theta;
  Op<2>() = Phi;
  Op<3>() = Lambda;
...
UGateInst::UGateInst( Value *Qubit, Value *Theta, Value *Phi, Value *Lambda, Value* Qubit1, Value* Qubit2, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), UGate, OperandTraits<UGateInst>::op_begin(this), 6, InsertAtEnd){
  Op<0>() = Qubit;
  Op<1>() = Theta;
  Op<2>() = Phi;
  Op<3>() = Lambda;
...
UGateInst *UGateInst::cloneImpl() const {
  return new UGateInst(getOperand(0), getOperand(1), getOperand(2), getOperand(3));
}


//===----------------------------------------------------------------------===//
//                      IGateInst Implementation
//===----------------------------------------------------------------------===//


IGateInst::IGateInst(Value *Qubit, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), IGate, OperandTraits<IGateInst>::op_begin(this), 1, InsertBef){
   Op<0>() = Qubit;

  setName(Name);
}

IGateInst::IGateInst( Value *Qubit, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), IGate, OperandTraits<IGateInst>::op_begin(this), 1, InsertAtEnd){
...
IGateInst *IGateInst::cloneImpl() const {
  return new IGateInst(getOperand(0));
}


//===----------------------------------------------------------------------===//
//                      TGateInst Implementation
//===----------------------------------------------------------------------===//


TGateInst::TGateInst(Value *Qubit, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), TGate, OperandTraits<TGateInst>::op_begin(this), 1, InsertBef){
   Op<0>() = Qubit;

  setName(Name);
}

TGateInst::TGateInst( Value *Qubit, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), TGate, OperandTraits<TGateInst>::op_begin(this), 1, InsertAtEnd){
...
TGateInst *TGateInst::cloneImpl() const {
  return new TGateInst(getOperand(0));
} 


//===----------------------------------------------------------------------===//
//                      TdagGateInst Implementation
//===----------------------------------------------------------------------===//


TdagGateInst::TdagGateInst(Value *Qubit, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), TdagGate, OperandTraits<TdagGateInst>::op_begin(this), 1, InsertBef){
   Op<0>() = Qubit;

  setName(Name);
}

TdagGateInst::TdagGateInst( Value *Qubit, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), TdagGate, OperandTraits<TdagGateInst>::op_begin(this), 1, InsertAtEnd){
...
TdagGateInst *TdagGateInst::cloneImpl() const {
  return new TdagGateInst(getOperand(0));
}

//===----------------------------------------------------------------------===//
//                      SGateInst Implementation
//===----------------------------------------------------------------------===//


SGateInst::SGateInst(Value *Qubit, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), SGate, OperandTraits<SGateInst>::op_begin(this), 1, InsertBef){
   Op<0>() = Qubit;

  setName(Name);
}

SGateInst::SGateInst( Value *Qubit, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), SGate, OperandTraits<SGateInst>::op_begin(this), 1, InsertAtEnd){
...
SGateInst *SGateInst::cloneImpl() const {
  return new SGateInst(getOperand(0));
}


//===----------------------------------------------------------------------===//
//                      SxGateInst Implementation
//===----------------------------------------------------------------------===//


SxGateInst::SxGateInst(Value *Qubit, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), SxGate, OperandTraits<SxGateInst>::op_begin(this), 1, InsertBef){
   Op<0>() = Qubit;

  setName(Name);
}

SxGateInst::SxGateInst( Value *Qubit, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), SxGate, OperandTraits<SxGateInst>::op_begin(this), 1, InsertAtEnd){
...
SxGateInst *SxGateInst::cloneImpl() const {
  return new SxGateInst(getOperand(0));
}

//===----------------------------------------------------------------------===//
//                      SdagGateInst Implementation
//===----------------------------------------------------------------------===//


SdagGateInst::SdagGateInst(Value *Qubit, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), SdagGate, OperandTraits<SdagGateInst>::op_begin(this), 1, InsertBef){
   Op<0>() = Qubit;

  setName(Name);
}

SdagGateInst::SdagGateInst( Value *Qubit, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), SdagGate, OperandTraits<SdagGateInst>::op_begin(this), 1, InsertAtEnd){
...
SdagGateInst *SdagGateInst::cloneImpl() const {
  return new SdagGateInst(getOperand(0));
}

//===----------------------------------------------------------------------===//
//                      RotationGateInst Implementation
//===----------------------------------------------------------------------===//
void RotationGateInst::initRotationValue(Value *Val){
  if (ConstantFP* constantFP = dyn_cast<ConstantFP>(Val)) {
    rotationValue = (constantFP->getValueAPF()).convertToDouble();
  }
}


//===----------------------------------------------------------------------===//
//                      RxGateInst Implementation
//===----------------------------------------------------------------------===//


RxGateInst::RxGateInst(Value *Qubit,Value *Rotate, const Twine &Name, Instruction *InsertBef)
: RotationGateInst(Qubit->getType(), RxGate, OperandTraits<RxGateInst>::op_begin(this), 2, InsertBef){
   
  Op<0>() = Qubit;
  Op<1>() = Rotate;
  initRotationValue(Rotate);
  setName(Name);
}

RxGateInst::RxGateInst( Value *Qubit,Value *Rotate, const Twine &NameStr, BasicBlock *InsertAtEnd)
: RotationGateInst(Qubit->getType(), RxGate, OperandTraits<RxGateInst>::op_begin(this), 2, InsertAtEnd){
...
RxGateInst *RxGateInst::cloneImpl() const {
  return new RxGateInst(getOperand(0), getOperand(1));
}


//===----------------------------------------------------------------------===//
//                      RyGateInst Implementation
//===----------------------------------------------------------------------===//


RyGateInst::RyGateInst(Value *Qubit,Value *Rotate, const Twine &Name, Instruction *InsertBef)
: RotationGateInst(Qubit->getType(), RyGate, OperandTraits<RyGateInst>::op_begin(this), 2, InsertBef){
   Op<0>() = Qubit;
   Op<1>() = Rotate;
   initRotationValue(Rotate);
   setName(Name);
...
RyGateInst::RyGateInst( Value *Qubit,Value *Rotate, const Twine &NameStr, BasicBlock *InsertAtEnd)
: RotationGateInst(Qubit->getType(), RyGate, OperandTraits<RyGateInst>::op_begin(this), 2, InsertAtEnd){
  Op<0>() = Qubit;
  Op<1>() = Rotate;
  initRotationValue(Rotate);
  setName(NameStr);
...
RyGateInst *RyGateInst::cloneImpl() const {
  return new RyGateInst(getOperand(0), getOperand(1));
} 


//===----------------------------------------------------------------------===//
//                      RzGateInst Implementation
//===----------------------------------------------------------------------===//


RzGateInst::RzGateInst(Value *Qubit,Value *Rotate, const Twine &Name, Instruction *InsertBef)
: RotationGateInst(Qubit->getType(), RzGate, OperandTraits<RzGateInst>::op_begin(this), 2, InsertBef){
   Op<0>() = Qubit;
   Op<1>() = Rotate;
   initRotationValue(Rotate);
  setName(Name);
...
RzGateInst::RzGateInst( Value *Qubit,Value *Rotate, const Twine &NameStr, BasicBlock *InsertAtEnd)
: RotationGateInst(Qubit->getType(), RzGate, OperandTraits<RzGateInst>::op_begin(this), 2, InsertAtEnd){
  Op<0>() = Qubit;
  Op<1>() = Rotate;
  initRotationValue(Rotate);
  setName(NameStr);
...
RzGateInst *RzGateInst::cloneImpl() const {
  return new RzGateInst(getOperand(0), getOperand(1));
}

//===----------------------------------------------------------------------===//
//                      CXGate Implementation
//===----------------------------------------------------------------------===//

void MultiGateInst::setQubitID(Value *QID){

  if (ConstantInt* CI = dyn_cast<ConstantInt>(QID)) {
    if (CI->getBitWidth() <= 32) {
      QubitID = CI->getSExtValue();
    }
  }
}

//===----------------------------------------------------------------------===//
//                      CX0GateInst Implementation
//===----------------------------------------------------------------------===//


CX0GateInst::CX0GateInst(Value *Qubit,Value *QID, const Twine &Name, Instruction *InsertBef)
: CXGateInst(Qubit->getType(), CX0Gate, OperandTraits<CX0GateInst>::op_begin(this), 2, InsertBef){
   Op<0>() = Qubit;
   Op<1>() = QID;
...
CX0GateInst::CX0GateInst( Value *Qubit,Value *QID, const Twine &NameStr, BasicBlock *InsertAtEnd)
: CXGateInst(Qubit->getType(), CX0Gate, OperandTraits<CX0GateInst>::op_begin(this), 2, InsertAtEnd){
  Op<0>() = Qubit;
  Op<1>() = QID;
...
CX0GateInst *CX0GateInst::cloneImpl() const {
  return new CX0GateInst(getOperand(0), getOperand(1));
}


//===----------------------------------------------------------------------===//
//                      CX1GateInst Implementation
//===----------------------------------------------------------------------===//


CX1GateInst::CX1GateInst(Value *Qubit,Value *QID, const Twine &Name, Instruction *InsertBef)
: CXGateInst(Qubit->getType(), CX1Gate, OperandTraits<CX1GateInst>::op_begin(this), 2, InsertBef){
  Op<0>() = Qubit;
  Op<1>() = QID;
...
CX1GateInst::CX1GateInst( Value *Qubit,Value *QID, const Twine &NameStr, BasicBlock *InsertAtEnd)
: CXGateInst(Qubit->getType(), CX1Gate, OperandTraits<CX1GateInst>::op_begin(this), 2, InsertAtEnd){
  Op<1>() = QID;  
  Op<0>() = Qubit;
...
CX1GateInst *CX1GateInst::cloneImpl() const {
  return new CX1GateInst(getOperand(0), getOperand(1));
}

//===----------------------------------------------------------------------===//
//                      CtrlGateInst Implementation
//===----------------------------------------------------------------------===//


CtrlGateInst::CtrlGateInst(Value *Qubit, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(),CtrlGate, OperandTraits<CtrlGateInst>::op_begin(this), 1, InsertBef){
   Op<0>() = Qubit;
...
CtrlGateInst::CtrlGateInst( Value *Qubit, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(),CtrlGate, OperandTraits<CtrlGateInst>::op_begin(this), 1, InsertAtEnd){
  Op<0>() = Qubit;

  setName(NameStr);
}

CtrlGateInst *CtrlGateInst::cloneImpl() const {
  return new CtrlGateInst(getOperand(0));
}

//===----------------------------------------------------------------------===//
//                      MeasXGateInst Implementation
//===----------------------------------------------------------------------===//


MeasXGateInst::MeasXGateInst(Value *Qubit, const Twine &Name, Instruction *InsertBef)
: GateInst(Type::getInt32Ty(Qubit->getContext()), MeasXGate, OperandTraits<MeasXGateInst>::op_begin(this), 1, InsertBef){
   Op<0>() = Qubit;
  setName(Name);
}

MeasXGateInst::MeasXGateInst( Value *Qubit, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Type::getInt32Ty(Qubit->getContext()), MeasXGate, OperandTraits<MeasXGateInst>::op_begin(this), 1, InsertAtEnd){
  
  Op<0>() = Qubit;
  setName(NameStr);
}

MeasXGateInst *MeasXGateInst::cloneImpl() const {
  return new MeasXGateInst(getOperand(0));
}


//===----------------------------------------------------------------------===//
//                      MeasYGateInst Implementation
//===----------------------------------------------------------------------===//


MeasYGateInst::MeasYGateInst(Value *Qubit, const Twine &Name, Instruction *InsertBef)
: GateInst(Type::getInt32Ty(Qubit->getContext()), MeasYGate, OperandTraits<MeasYGateInst>::op_begin(this), 1, InsertBef){
   Op<0>() = Qubit;
  setName(Name);
}

MeasYGateInst::MeasYGateInst( Value *Qubit, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Type::getInt32Ty(Qubit->getContext()), MeasYGate, OperandTraits<MeasYGateInst>::op_begin(this), 1, InsertAtEnd){
  Op<0>() = Qubit;
  setName(NameStr);
}

MeasYGateInst *MeasYGateInst::cloneImpl() const {
  return new MeasYGateInst(getOperand(0));
}


//===----------------------------------------------------------------------===//
//                      MeasZGateInst Implementation
//===----------------------------------------------------------------------===//


MeasZGateInst::MeasZGateInst(Value *Qubit, const Twine &Name, Instruction *InsertBef)
: GateInst(Type::getInt32Ty(Qubit->getContext()), MeasZGate, OperandTraits<MeasZGateInst>::op_begin(this), 1, InsertBef){
   Op<0>() = Qubit;
  setName(Name);
}

MeasZGateInst::MeasZGateInst( Value *Qubit, const Twine &Name, BasicBlock *InsertAtEnd)
: GateInst(Type::getInt32Ty(Qubit->getContext()), MeasZGate, OperandTraits<MeasZGateInst>::op_begin(this), 1, InsertAtEnd){
  Op<0>() = Qubit;
  setName(Name);
}

MeasZGateInst *MeasZGateInst::cloneImpl() const {
  return new MeasZGateInst(getOperand(0));
}


//===----------------------------------------------------------------------===//
//                      SwapGateInst Implementation
//===----------------------------------------------------------------------===//


SwapGateInst::SwapGateInst(Value *Qubit,Value *Ctrl, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), SwapGate, OperandTraits<SwapGateInst>::op_begin(this), 2, InsertBef){
...
SwapGateInst::SwapGateInst(Value *Qubit,Value *Ctrl, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), SwapGate, OperandTraits<SwapGateInst>::op_begin(this), 2, InsertAtEnd){
...
SwapGateInst::SwapGateInst(Value *Qubit,Value *Qubit1, Value *Ctrl, const Twine &Name, Instruction *InsertBef)
: GateInst(Qubit->getType(), SwapGate, OperandTraits<SwapGateInst>::op_begin(this), 3, InsertBef){
...
SwapGateInst::SwapGateInst(Value *Qubit,Value *Qubit1, Value *Ctrl, const Twine &NameStr, BasicBlock *InsertAtEnd)
: GateInst(Qubit->getType(), SwapGate, OperandTraits<SwapGateInst>::op_begin(this), 3, InsertAtEnd){
...
SwapGateInst *SwapGateInst::cloneImpl() const {
  return new SwapGateInst(getOperand(0), getOperand(1));
}
