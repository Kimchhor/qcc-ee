...
Type *Type::getPrimitiveType(LLVMContext &C, TypeID IDNumber) {
  ...
  case QubitTyID     : return getQubitTy(C); // QIR HPCL: Qubit Type
  default:
    return nullptr;
  }
}
...
TypeSize Type::getPrimitiveSizeInBits() const {
  ...
  case Type::QubitTyID: return TypeSize::Fixed(32); // QIR HPCL
...
// QIR HPCL
Type *Type::getQubitTy(LLVMContext &C) { 


  // QIR HPCL: Testing
  Type *Entry = &C.pImpl->QubitTy;
  if (!Entry){
    errs() << "Not Yet init\n";
    Entry = new (C.pImpl->Alloc) QubitType(C, 64);
    errs() << "Type::getQubitTy: " << Entry << "\n";

  }
  // End: Testing


  return &C.pImpl->QubitTy;
}
...
PointerType *Type::getQubitPtrTy(LLVMContext &C, unsigned AS) {
  return getQubitTy(C)->getPointerTo(AS);
}
...
//===----------------------------------------------------------------------===//
// QIR HPCL
int QubitType::getNumberOfGate() {
  return 0;
}

QubitType *QubitType::get(LLVMContext &C, unsigned NumBits) {
  // Will update if qubit support numbits
  return cast<QubitType>(Type::getQubitTy(C));
}