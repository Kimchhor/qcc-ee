...
// Test QCC specific measure X.

void test_measureX_1() {
    // CHECK-LABEL: define dso_local void @test_measureX_1() #0 {
    // CHECK: entry:
...
void test_measureX_2() {
    // CHECK-LABEL: define dso_local void @test_measureX_2() #0 {
    // CHECK: entry:
    ...
    // CHECK: %0 = load q1, q1* %q, align 4
    // CHECK: %1 = measure_x q1 %0
    // CHECK: store i32 %1, i32* %c, align 4
    c = measureX -> q;

    // CHECK: %2 = load q1, q1* %q1, align 4
    // CHECK: %3 = measure_x q1 %2
