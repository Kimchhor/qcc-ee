...
#undef RTT_JOIN
  ...
  // QCL QCC - gate : perform parser analysis
  ...
  case tok::kw_h:
  case tok::kw_p: 
  case tok::kw_cnot: // 2-qubit gates
  case tok::kw_swap:
  case tok::kw_cx:
  case tok::kw_cy:
  case tok::kw_cz:
  ...
  case tok::kw_cry:
  case tok::kw_crz:
  case tok::kw_ccx:  // 3-qubit gates
  case tok::kw_ccy:
  case tok::kw_ccz:
  case tok::kw_ccu:
  case tok::kw_cswap:
  case tok::kw_measure: // measurement
  case tok::kw_measureX:
  case tok::kw_measureY:
  {
    ...
    Token SavedTok = Tok; // Save 'cnot'

    ConsumeToken(); // Eat 'cnot'

      // Check whether it arrow
    if (Tok.isNot(tok::arrow)) {
      Diag(Tok, diag::err_qcc_requires_expr_missing_arrow);
        return ExprError();
    }
    ...
    // Cast and Eat 'q1'
    Res = ParseCastExpression(AnyCastExpr, false, NotCastExpr, NotTypeCast);
    ...
    while (Tok.is(tok::comma)){
      ...
      ExprResult ResArg = ParseCastExpression(AnyCastExpr, false, NotCastExpr, NotTypeCast);

      if (NotCastExpr) {
        // If we return with NotCastExpr = true, we must not consume any tokens,
        // so put the token back where we found it.
        assert(ResArg.isInvalid());
        UnconsumeToken(SavedTok);
        return ExprError();
      }
      if (!ResArg.isInvalid())
        Args.push_back(ResArg.get());
      else
        return ExprError();
    }

    ArrayRef<Expr *> ArgsRef = ArrayRef<Expr *>(Args);

    if (!Res.isInvalid()){
       Expr *ArgFirst = Res.get();
      Res = Actions.ActOnQuantumOp(getCurScope(), SavedTok.getLocation(), SavedKind, ArgFirst, ArgsRef);
    }else{
      return ExprError();
    }
    return Res;
