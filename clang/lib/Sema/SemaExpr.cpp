...
// QCL QCC - gate : this method is used to convert token type to quantum operation code
static inline QuantumOperatorKind ConvertTokenKindToQuantumOpcode(
  tok::TokenKind Kind) {
  QuantumOperatorKind Opc;
  switch (Kind) {
  default: llvm_unreachable("Unknown quantum op!");
  case tok::kw_prez:        Opc = QO_InitZ; break;
  case tok::kw_prex:        Opc = QO_InitX; break;

  // 1-qubit gates
  case tok::kw_x:           Opc = QO_X; break;
  case tok::kw_y:           Opc = QO_Y; break;
  case tok::kw_z:           Opc = QO_Z; break;
  
  case tok::kw_rx:          Opc = QO_X; break;
  case tok::kw_ry:          Opc = QO_Y; break;
  case tok::kw_rz:          Opc = QO_Z; break;
  case tok::kw_p:           Opc = QO_P; break;
  case tok::kw_u:           Opc = QO_U; break;

  case tok::kw_idn:         Opc = QO_I; break;
  case tok::kw_h:           Opc = QO_H; break;
  case tok::kw_s:           Opc = QO_S; break;
  case tok::kw_sdag:        Opc = QO_Sdag; break;
  case tok::kw_t:           Opc = QO_T; break;
  case tok::kw_tdag:        Opc = QO_Tdag; break;
  case tok::kw_sx:          Opc = QO_Sx; break;

  // 2-qubit gates
  case tok::kw_swap:        Opc = QO_Swap; break;
  case tok::kw_cnot:        Opc = QO_Cx; break;
  
  case tok::kw_cx:          Opc = QO_Cx; break;
  case tok::kw_cy:          Opc = QO_Cy; break;
  case tok::kw_cz:          Opc = QO_Cz; break;
  case tok::kw_cp:          Opc = QO_Cp; break;

  case tok::kw_ch:          Opc = QO_Ch; break;
  case tok::kw_cu:          Opc = QO_Cu; break;

  case tok::kw_crx:         Opc = QO_Crx; break;
  case tok::kw_cry:         Opc = QO_Cry; break;
  case tok::kw_crz:         Opc = QO_Crz; break;
  
  // 3-qubit gates
  case tok::kw_ccx:         Opc = QO_Ccx; break;
  case tok::kw_ccy:         Opc = QO_Ccy; break;
  case tok::kw_ccz:         Opc = QO_Ccz; break;
  case tok::kw_ccu:         Opc = QO_Ccu; break;
  
  case tok::kw_cswap:       Opc = QO_Cswap; break;

  // measurement
  case tok::kw_measure:     Opc = QO_Measure; break;
  case tok::kw_measureY:    Opc = QO_MeasureY; break;
  case tok::kw_measureX:    Opc = QO_MeasureX; break;
  }
  return Opc;
}
...
// QCL QCC - gate : TODO - return the return type of expr
ExprResult Sema::BuildQuantumOp(Scope *S, SourceLocation OpLoc,
                          QuantumOperatorKind Opc, Expr *InputExpr){
  ...
  switch (Opc) {
    ...
    case QO_Rz:
    case QO_H:{
      resultType = Input.get()->getType().getUnqualifiedType();
      break;
    case QO_Measure:
    case QO_MeasureX:
    case QO_MeasureY:
    ...
    default:{
      assert(!"BuildQuantumOp with 1 input should be called for single-qubit gate only.");
    }
  }
  ...
  if (const BuiltinType *pty = InputExpr->getType()->getAsPlaceholderType()) {
    // Anything else needs to be handled now.
    ExprResult Result = CheckPlaceholderExpr(InputExpr);
    if (Result.isInvalid()) return ExprError();
    InputExpr = Result.get();
  }

  auto *QO = QuantumOperator::Create(Context, Input.get(), Opc, resultType, VK, OK, OpLoc);

  return QO;
}

ExprResult Sema::BuildQuantumOp(Scope *S, SourceLocation OpLoc,
                          QuantumOperatorKind Opc, Expr *FInputExpr, 
                          Expr *SInputExpr){
  
  ExprResult FInput  = FInputExpr;
  ExprResult SInput = SInputExpr;
  ExprValueKind VK = VK_RValue; 
  ExprObjectKind OK = OK_Ordinary;
  QualType resultType;
  ...
  switch (Opc) {
    ...
    case QO_U:
    case QO_Z:{
      resultType = FInput.get()->getType().getUnqualifiedType();
      break;
    }
    case QO_Swap:{
      resultType = SInput.get()->getType().getUnqualifiedType();
      break;
    }
    default:{
      assert(!"BuildQuantumOp with 1 input should be called for single-qubit gate only.");
  ...
  if (resultType.isNull() || FInput.isInvalid() || SInput.isInvalid())
    return ExprError();

  if (const BuiltinType *pty = FInputExpr->getType()->getAsPlaceholderType()) {
    // Anything else needs to be handled now.
    ExprResult Result = CheckPlaceholderExpr(FInputExpr);
    if (Result.isInvalid()) return ExprError();
    FInputExpr = Result.get();
  }

  auto *QO = QuantumOperator::Create(Context, FInput.get(), Opc, resultType, VK, OK, OpLoc);
...
ExprResult Sema::BuildQuantumOp(Scope *S, SourceLocation OpLoc,
                          QuantumOperatorKind Opc, Expr *FInputExpr, 
                          Expr *SInputExpr, Expr *TInputExpr){
  
  ExprResult FInput  = FInputExpr;
  ExprResult SInput = SInputExpr;
  ExprResult TInput = TInputExpr;
  ...
  switch (Opc) {
    ...
    default:{
      assert(!"BuildQuantumOp with 3 input should be called for single-qubit gate only.");
    }
  }

  if (resultType.isNull() || FInput.isInvalid() || SInput.isInvalid() || TInput.isInvalid())
    return ExprError();
...
ExprResult Sema::BuildQuantumOp(Scope *S, SourceLocation OpLoc,
                          QuantumOperatorKind Opc, Expr *FInputExpr, 
                          Expr *SInputExpr, Expr *TInputExpr, Expr *FourthInputExpr){
  
  ExprResult FInput  = FInputExpr;
  ExprResult SInput = SInputExpr;
  ExprResult TInput = TInputExpr;
  ExprResult FourthInput = FourthInputExpr;
  ...
  switch (Opc) {
    ...
    default:{
      assert(!"BuildQuantumOp with 4 input should be called for single-qubit gate only.");
    }
  }

  if (resultType.isNull() || FInput.isInvalid() || SInput.isInvalid() || TInput.isInvalid() || FourthInput.isInvalid())
    return ExprError();
...
ExprResult Sema::BuildQuantumOp(Scope *S, SourceLocation OpLoc,
                          QuantumOperatorKind Opc, Expr *FInputExpr, ArrayRef<Expr *> Inputs){
  ExprResult FInput  = FInputExpr;
  ...
  switch (Opc){
    ...
    case QO_Crz:
    case QO_Swap:{
      resultType = Inputs[0]->getType().getUnqualifiedType();
      break;
    }
    case QO_Ccx:
    case QO_Ccy:
    ...
    case QO_Ccu:
    case QO_Cswap:{
      resultType = Inputs[1]->getType().getUnqualifiedType();
      break;
    }
    case QO_Measure:
    case QO_MeasureX:
    ...
    default:{
      assert(!"BuildQuantumOp is not supported.");
    }
  }

  if (resultType.isNull() || FInput.isInvalid())
    return ExprError();

  auto *QO = QuantumOperator::Create(Context, FInput.get(), Inputs, Opc, resultType, VK, OK, OpLoc);
  return QO;
}

ExprResult Sema::ActOnQuantumOp(Scope *S, SourceLocation OpLoc,
                          tok::TokenKind Op, Expr *FirstInput, Expr *SecondInput, Expr *ThirdInput, Expr *FourthInput){
                          
  if (FourthInput)
    return BuildQuantumOp(S, OpLoc, ConvertTokenKindToQuantumOpcode(Op), FirstInput, SecondInput, ThirdInput, FourthInput);
  if (ThirdInput)
    return BuildQuantumOp(S, OpLoc, ConvertTokenKindToQuantumOpcode(Op), FirstInput, SecondInput, ThirdInput);
  if (SecondInput)
    return BuildQuantumOp(S, OpLoc, ConvertTokenKindToQuantumOpcode(Op), FirstInput, SecondInput);

  return BuildQuantumOp(S, OpLoc, ConvertTokenKindToQuantumOpcode(Op), FirstInput);
}

ExprResult Sema::ActOnQuantumOp(Scope *S, SourceLocation OpLoc,
                          tok::TokenKind Op, Expr *FirstInput, ArrayRef<Expr *> Inputs){

  return BuildQuantumOp(S, OpLoc, ConvertTokenKindToQuantumOpcode(Op), FirstInput, Inputs);
}
...
namespace {
  ...
  public:
    ...
    // QCL QCC - gate
    ExprResult TransformQuantumOperator(QuantumOperator *E) {

      return BaseTransform::TransformQuantumOperator(E);
    }
