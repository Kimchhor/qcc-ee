...
#include "llvm/Frontend/OpenMP/OMP.inc"
  ...
  /// Build a new quantum operator expression.
  ///
  /// By default, performs semantic analysis to build the new expression.
  /// Subclasses may override this routine to provide different behavior.
  ExprResult RebuildQuantumOperator(SourceLocation OpLoc,
                                        QuantumOperatorKind Opc,
                                        Expr *SubExpr) {
    return getSema().BuildQuantumOp(/*Scope=*/nullptr, OpLoc, Opc, SubExpr);
  }
...
template<typename Derived>
ExprResult
TreeTransform<Derived>::TransformQuantumOperator(QuantumOperator *E) {
  ExprResult SubExpr;

  SubExpr = TransformExpr(E->getSubExprVal());
  SmallVector<Expr*, 8> SubExprsParams;

  bool ArgumentChanged = false;
  SubExprsParams.reserve(E->getNumSubExprs());

  if (SubExpr.isInvalid())
    return ExprError();

  if (!getDerived().AlwaysRebuild() && SubExpr.get() == E->getSubExprVal())
    return E;

  // return getDerived().RebuildShuffleVectorExpr(E->getBuiltinLoc(), SubExprsParams, E->getRParenLoc());
  return getDerived().RebuildQuantumOperator(E->getOperatorLoc(), E->getOpcode(), SubExpr.get());
}
