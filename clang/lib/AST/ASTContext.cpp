...
#include "clang/Basic/RISCVVTypes.def"
  ...
  // QCL QCC - type : qcc types are added for initilization
  /** qbit, cbit and abit types are added for initalization */
...
unsigned ASTContext::getIntegerRank(const Type *T) const {
  ...
  // QCL QCC - type : rank cbit and qubit as a result
  /**  It will use rank for cbit and qubit as a result */ 
  case BuiltinType::Bool:
...
static char getObjCEncodingForPrimitiveType(const ASTContext *C,
    ...
    // QCL QCC - type
    case BuiltinType::Qbit:       return 'u';
    case BuiltinType::Cbit:       return 'x';
    case BuiltinType::Abit:       return 'e';
...
static QualType DecodeTypeFromStr(const char *&Str, const ASTContext &Context,
  ...
  // QCL QCC - type 
  /**Quantum type for builtin gate
   * u : Qubit type
   * x : Classical bit type
   * e : Ancilla type 
  */
  case 'u':
    assert(HowLong == 0 && !Signed && !Unsigned &&
           "Bad modifiers used with 'u'!");
    Type = Context.QbitTy;
    break;
  case 'x':
    assert(HowLong == 0 && !Signed && !Unsigned &&
