...
bool Expr::isUnusedResultAWarning(const Expr *&WarnE, SourceLocation &Loc,
  ...
  case ChooseExprClass:
    return cast<ChooseExpr>(this)->getChosenSubExpr()->
      isUnusedResultAWarning(WarnE, Loc, R1, R2, Ctx);
  // QCL QCC - gate : determine unused result warning
  case QuantumOperatorClass: {
    return false;
  }
...
bool Expr::HasSideEffects(const ASTContext &Ctx,
  ...
  case CoyieldExprClass:
    // QCL QCC - gate : determine if gate has side effects
  case QuantumOperatorClass:
...
// QCL QCC - gate : implement QuantumOperator class
/** Implement QuantumOperator class*/
QuantumOperator *QuantumOperator::CreateEmpty(const ASTContext &C){
  void *Mem = C.Allocate(sizeof(QuantumOperator), alignof(QuantumOperator));
  return new (Mem) QuantumOperator(EmptyShell());
}

QuantumOperator::QuantumOperator(const ASTContext &Ctx, Expr *input, Opcode opc, 
                                QualType type, ExprValueKind VK, 
                                ExprObjectKind OK, SourceLocation l)
    : Expr(QuantumOperatorClass, type, VK, OK), SubExpr(input){
    QuantumOperatorBits.Opc = opc;
    QuantumOperatorBits.Loc = l;
    setDependence(this->getSubExprVal()->getDependence());
}


QuantumOperator::QuantumOperator(const ASTContext &Ctx, Expr *inputFirst, ArrayRef<Expr *> params, Opcode opc, QualType type,
                    ExprValueKind VK, ExprObjectKind OK, SourceLocation l): Expr(QuantumOperatorClass, type, VK, OK), SubExpr(inputFirst){
    


  this->NumExprs = params.size();

  SubExprParams = new (Ctx) Stmt*[params.size()];
  
  for (unsigned i = 0; i != params.size(); i++)
    SubExprParams[i] = params[i];
...
QuantumOperator::QuantumOperator(const ASTContext &Ctx, Expr *inputFirst, ArrayRef<Stmt *> params, Opcode opc, QualType type,
                    ExprValueKind VK, ExprObjectKind OK, SourceLocation l): Expr(QuantumOperatorClass, type, VK, OK), SubExpr(inputFirst){
...
QuantumOperator *QuantumOperator::Create(const ASTContext &C, Expr *input, Opcode opc,
                                         QualType type, ExprValueKind VK,
                                         ExprObjectKind OK, SourceLocation l){
  void *Mem = C.Allocate(sizeof(QuantumOperator), alignof(QuantumOperator));
  return new (Mem)
      QuantumOperator(C, input, opc, type, VK, OK, l);
}

QuantumOperator *QuantumOperator::Create(const ASTContext &C, Expr *inputFirst, ArrayRef<Expr *> params, 
                                         Opcode opc, QualType type, ExprValueKind VK,
                                         ExprObjectKind OK, SourceLocation l){
  
  void *Mem = C.Allocate(sizeof(QuantumOperator), alignof(QuantumOperator));
  return new (Mem)
      QuantumOperator(C, inputFirst, params, opc, type, VK, OK, l);
}

QuantumOperator *QuantumOperator::Create(const ASTContext &C, Expr *inputFirst, ArrayRef<Stmt *> params, 
                                         Opcode opc, QualType type, ExprValueKind VK,
                                         ExprObjectKind OK, SourceLocation l){
  void *Mem = C.Allocate(sizeof(QuantumOperator), alignof(QuantumOperator));
  return new (Mem)
 ...
 void QuantumOperator::setExprs(const ASTContext &Ctx, ArrayRef<Expr *> Exprs){
      if (SubExprParams) Ctx.Deallocate(SubExprParams);

      this->NumExprs = Exprs.size();
      SubExprParams = new (Ctx) Stmt*[NumExprs];
      memcpy(SubExprParams, Exprs.data(), sizeof(Expr *) * Exprs.size());
    }

unsigned QuantumOperator::QubitID = 1;

/// getOpcodeStr - Turn an Opcode enum value into the punctuation char it
/// corresponds to, e.g. "sizeof"
StringRef QuantumOperator::getOpcodeStr(Opcode Op){
  switch(Op){
    #define QUANTUM_OPERATION(Name) case QO_##Name : return #Name;
    #include "clang/AST/OperationKinds.def"
  }
  llvm_unreachable("Unknown quantum operator");
}
