...
// QCL QCC - gate
/** Add printing support for quantum expression */
void StmtPrinter::VisitQuantumOperator(QuantumOperator *Node){
  OS << QuantumOperator::getOpcodeStr(Node->getOpcode());

  OS << "__builtin_quantumvector(";
  PrintExpr(Node->getSubExprVal());
  for (unsigned i = 0, e = Node->getNumSubExprs(); i != e; ++i) {
    if (i) OS << ", ";
    PrintExpr(Node->getSubExpr(i));
  }
  OS << ")";
}
