...
namespace clang {
  ...
  public:
    ...
    // QCL QCC - gate
    ExpectedStmt VisitQuantumSingleQubitGate(QuantumOperator *E);
    ExpectedStmt VisitQuantumMultiQubitGate(QuantumOperator *E);
    ExpectedStmt VisitQuantumMeasureGate(QuantumOperator *E);
...
// QCL QCC - gate
ExpectedStmt ASTNodeImporter::VisitQuantumSingleQubitGate(QuantumOperator *E){
  Error Err = Error::success();
  auto ToSubExpr = importChecked(Err, E->getSubExprVal());
  auto ToType = importChecked(Err, E->getType());
  auto ToOperatorLoc = importChecked(Err, E->getOperatorLoc());
  if (Err)
    return std::move(Err);

  return QuantumOperator::Create(
      Importer.getToContext(), ToSubExpr, E->getOpcode(), ToType,
      E->getValueKind(), E->getObjectKind(), ToOperatorLoc);

}

ExpectedStmt ASTNodeImporter::VisitQuantumMultiQubitGate(QuantumOperator *E){
  Error Err = Error::success();
  auto ToSubExpr = importChecked(Err, E->getSubExprVal());
  
  SmallVector<Stmt *, 0> StmtParams;

  for (unsigned i = 0, e = E->getNumSubExprs(); i != e; ++i) {
    StmtParams.push_back(importChecked(Err, E->getSubExpr(i)));
  }

  auto ToSubExprParams = ArrayRef<Stmt *>(StmtParams);
  ...
  return QuantumOperator::Create(
      Importer.getToContext(), ToSubExpr, ToSubExprParams, E->getOpcode(), ToType,
      E->getValueKind(), E->getObjectKind(), ToOperatorLoc);

}

ExpectedStmt ASTNodeImporter::VisitQuantumMeasureGate(QuantumOperator *E){
  Error Err = Error::success();
  auto ToSubExpr = importChecked(Err, E->getSubExprVal());
  auto ToType = importChecked(Err, E->getType());
  auto ToOperatorLoc = importChecked(Err, E->getOperatorLoc());
