...
public:
  ...
  // QCL QCC - gate : TODO - this method is used for visting from all gates 

  /** This method is used for visiting from all single-qubit gate*/
  Value *VisitQuantumSingleQubitGate(const QuantumOperator *E){
    
    // Visit first operand
    Value *V = Visit(E->getSubExprVal());
    
    // It produces store instruction here
    LValue LV = CGF.EmitLValue(E->getSubExprVal());

    Value *Gate;

    // Visit second operand 
    //  e.g x -> q1, 0.2
    //  The second operand is c
    Value *V1 = nullptr;
    Value *V2 = nullptr;
    Value *V3 = nullptr;

    unsigned numSubExprs = E->getNumSubExprs();

    if (numSubExprs > 0)
      V1 = Visit(E->getSubExpr(0));

    if (numSubExprs > 1)
      V2 = Visit(E->getSubExpr(1));
    
    if (numSubExprs > 2)
      V3 = Visit(E->getSubExpr(2));


    if (E->getOpcode() > QO_InitX && E->getOpcode() < QO_Sx)
       assert("Not a single-qubit gate");
  
    switch (E->getOpcode()) {
      case QO_InitX:    Gate = Builder.CreateGateInitX(V);          break;
      case QO_InitZ:    Gate = Builder.CreateGateInitZ(V);          break;
      case QO_Sx:       Gate = Builder.CreateGateSx(V);             break;
      case QO_S:        Gate = Builder.CreateGateS(V);              break;
      case QO_Sdag:     Gate = Builder.CreateGateSdag(V);           break;
      case QO_T:        Gate = Builder.CreateGateT(V);              break;
      case QO_Tdag:     Gate = Builder.CreateGateTdag(V);           break;
      case QO_I:        Gate = Builder.CreateGateI(V);              break;
      case QO_X:        Gate = Builder.CreateGateX(V, V1);          break;
      case QO_Y:        Gate = Builder.CreateGateY(V, V1);          break;
      case QO_Z:        Gate = Builder.CreateGateZ(V, V1);          break;
      case QO_H:        Gate = Builder.CreateGateH(V, V1);          break;
      case QO_P:        Gate = Builder.CreateGateP(V, V1);          break;
      case QO_Rx:       Gate = Builder.CreateGateX(V, V1);          break;
      case QO_Ry:       Gate = Builder.CreateGateY(V, V1);          break;
      case QO_Rz:       Gate = Builder.CreateGateZ(V, V1);          break;
      case QO_U:        Gate = Builder.CreateGateU(V, V1, V2, V3);  break;
      default:
        assert("CGExprScalar - Gate is not supported");
        return nullptr;
      }
    // Emit store instruction
    CGF.EmitStoreThroughLValue(RValue::get(Gate), LV);
    // CGF.EmitStoreThroughBitfieldLValue(RValue::get(V), LV, &V); // NOT WORK

    return Gate;
  }

  /** This method is used for visiting from all multi-qubit gate*/
  Value *VisitQuantumMultiQubitGate(const QuantumOperator *E){
    
    // Visit first operand
    //  e.g x -> q1;
    //  The first operand is q1
    Value *V = Visit(E->getSubExprVal());

    // Visit second operand 
    //  e.g measure -> q1, c
    //  The second operand is c 
    Value *V1 = Visit(E->getSubExpr(0)); 

    // Get the Left values
    LValue LV0 = CGF.EmitLValue(E->getSubExprVal());
    LValue LV1;
    LValue Target;
    ...
    if (numSubExprs > 3)
      V4 = Visit(E->getSubExpr(3));

    if (numSubExprs > 4)
      V5 = Visit(E->getSubExpr(4));

    Value *CtrlGate;
    Value *CtrlGate2;
    Value *Gate;

    CtrlGate = Builder.CreateGateCtrl(V);
    CGF.EmitStoreThroughLValue(RValue::get(CtrlGate), LV0);

    // QCC: Deprecated - Fetch the Qubit ID 
    llvm::Constant *QubitID = llvm::ConstantInt::get(Builder.getInt32Ty(), E->getQubitID());
    
    // 2-qubit gates
    if (E->getOpcode() >= QO_Cx && E->getOpcode() <= QO_Crz){
      
      Target = CGF.EmitLValue(E->getSubExpr(0));

    }else if (E->getOpcode() >= QO_Ccx && E->getOpcode() <= QO_Cswap){ // 3-qubit gates
      
      CtrlGate2 = Builder.CreateGateCtrl(V1);
      Target = CGF.EmitLValue(E->getSubExpr(1));
      LV1 = CGF.EmitLValue(E->getSubExpr(0));
      CGF.EmitStoreThroughLValue(RValue::get(CtrlGate2), LV1);
    
    }else{
      assert("CGExprScalar - Gate is not supported - check the gate in OperationKinds.def");
      return nullptr;
    }
    
    switch (E->getOpcode())
    {
    case QO_Swap:   Gate = Builder.CreateGateSwap(CtrlGate, V1); break;
    case QO_Cx:     Gate = Builder.CreateGateX(CtrlGate, V1); break;
    case QO_Cy:     Gate = Builder.CreateGateY(CtrlGate, V1); break;
    case QO_Cz:     Gate = Builder.CreateGateZ(CtrlGate, V1); break;
    case QO_Cp:     Gate = Builder.CreateGateP(CtrlGate, V1, V2); break;

    case QO_Ch:     Gate = Builder.CreateGateH(CtrlGate, V1); break;
    case QO_Cu:     Gate = Builder.CreateGateU(CtrlGate, V1, V2, V3, V4); break;

    case QO_Cry:    Gate = Builder.CreateGateY(CtrlGate, V1, V2); break;
    case QO_Crx:    Gate = Builder.CreateGateX(CtrlGate, V1, V2); break;
    case QO_Crz:    Gate = Builder.CreateGateZ(CtrlGate, V1, V2); break;
    case QO_Ccx:    {
      Gate = Builder.CreateGateX(CtrlGate, CtrlGate2,V2); 
      break;
    }
    case QO_Ccy:    {
      Gate = Builder.CreateGateY(CtrlGate, CtrlGate2, V2); 
      break;
    }
    case QO_Ccz:    {
      Gate = Builder.CreateGateZ(CtrlGate, CtrlGate2, V2); 
      break;
    }
    case QO_Ccu:    {
      Gate = Builder.CreateGateU(CtrlGate, CtrlGate2, V2, V3, V4, V5);
      break;
    }
    case QO_Cswap:  {
      Gate = Builder.CreateGateSwap(CtrlGate, CtrlGate2, V2);
      break;
    }
    default:
      assert("CGExprScalar - Gate is not supported");
    ...
    CGF.EmitStoreThroughLValue(RValue::get(Gate), Target);

    return Gate;

  }

  /** This method is used for visiting from all measure gate*/
  Value *VisitQuantumMeasureGate(const QuantumOperator *E){

    // Visit and emit the value of Qubit and Cbit 
    // Load instructions occure here
    // Value *Qbit = Visit(E->getSubExpr());
    // Value *V = Visit(E->getSubExpr());
    LValue LV = EmitLValue(E->getSubExprVal());
    Value *VV = EmitLoadOfLValue(LV, E->getExprLoc());
    ...
    if (E->getOpcode() == QO_Measure){
        Gate = Builder.CreateGateMeasureZ(VV);
    }else if (E->getOpcode() == QO_MeasureX){
        Gate = Builder.CreateGateMeasureX(VV);
    }else if (E->getOpcode() == QO_MeasureY){
        Gate = Builder.CreateGateMeasureX(VV);
    }
