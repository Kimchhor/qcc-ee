...
// QCL QCC - gate : add serialization support for reading
/** Add serialization support for reading */
void ASTStmtReader::VisitQuantumOperator(QuantumOperator *E){
  VisitExpr(E);
  E->setSubExprVal(Record.readSubExpr());
  E->setOpcode((QuantumOperator::Opcode)Record.readInt());
  E->setOperatorLoc(readSourceLocation());
...
Stmt *ASTReader::ReadStmtFromStream(ModuleFile &F) {
  ...
  while (true) {
    ...
    // QCL QCC - gate
    case EXPR_QUANTUM_OPERATOR:
      S = QuantumOperator::CreateEmpty(Context);
      break;
