...
#define DISPATCH(NAME, CLASS) \
  ...
  RetTy Visit(PTR(Stmt) S, ParamTys... P) {
    ...
    else if (PTR(QuantumOperator) QuOp = dyn_cast<QuantumOperator>(S)){
       // QCL QCC - gate : each of gate will dipatch the fucntion (Visit*) in CGExprScalar.cpp
      switch (QuOp->getOpcode())
      {
      case QO_InitX:      DISPATCH(QuantumSingleQubitGate,    QuantumOperator);
      case QO_InitZ:      DISPATCH(QuantumSingleQubitGate,    QuantumOperator);
      case QO_X:          DISPATCH(QuantumSingleQubitGate,    QuantumOperator);
      case QO_Y:          DISPATCH(QuantumSingleQubitGate,    QuantumOperator);
      case QO_Z:          DISPATCH(QuantumSingleQubitGate,    QuantumOperator);
      case QO_I:          DISPATCH(QuantumSingleQubitGate,    QuantumOperator);
      case QO_H:          DISPATCH(QuantumSingleQubitGate,    QuantumOperator);
      case QO_S:          DISPATCH(QuantumSingleQubitGate,    QuantumOperator);
      case QO_Sdag:       DISPATCH(QuantumSingleQubitGate,    QuantumOperator);
      case QO_T:          DISPATCH(QuantumSingleQubitGate,    QuantumOperator);
      case QO_Tdag:       DISPATCH(QuantumSingleQubitGate,    QuantumOperator);
      case QO_Sx:         DISPATCH(QuantumSingleQubitGate,    QuantumOperator);

      case QO_Rx:         DISPATCH(QuantumSingleQubitGate,    QuantumOperator);
      case QO_Ry:         DISPATCH(QuantumSingleQubitGate,    QuantumOperator);
      case QO_Rz:         DISPATCH(QuantumSingleQubitGate,    QuantumOperator);

      case QO_P:          DISPATCH(QuantumSingleQubitGate,    QuantumOperator);
      case QO_U:          DISPATCH(QuantumSingleQubitGate,    QuantumOperator);

      // 2-qubit gates
      case QO_Swap:       DISPATCH(QuantumMultiQubitGate,     QuantumOperator);
      case QO_Cx:         DISPATCH(QuantumMultiQubitGate,     QuantumOperator);
      case QO_Cy:         DISPATCH(QuantumMultiQubitGate,     QuantumOperator);
      case QO_Cz:         DISPATCH(QuantumMultiQubitGate,     QuantumOperator);
      case QO_Cp:         DISPATCH(QuantumMultiQubitGate,     QuantumOperator);
      case QO_Ch:         DISPATCH(QuantumMultiQubitGate,     QuantumOperator);
      case QO_Cu:         DISPATCH(QuantumMultiQubitGate,     QuantumOperator);

      case QO_Crx:        DISPATCH(QuantumMultiQubitGate,     QuantumOperator);
      case QO_Cry:        DISPATCH(QuantumMultiQubitGate,     QuantumOperator);
      case QO_Crz:        DISPATCH(QuantumMultiQubitGate,     QuantumOperator);

      // 3-qubit gates
      case QO_Ccx:        DISPATCH(QuantumMultiQubitGate,     QuantumOperator);
      case QO_Ccy:        DISPATCH(QuantumMultiQubitGate,     QuantumOperator);
      case QO_Ccz:        DISPATCH(QuantumMultiQubitGate,     QuantumOperator);
      case QO_Ccu:        DISPATCH(QuantumMultiQubitGate,     QuantumOperator);
      case QO_Cswap:      DISPATCH(QuantumMultiQubitGate,     QuantumOperator);
      
      // measurements
      case QO_Measure:    DISPATCH(QuantumMeasureGate,        QuantumOperator);
      case QO_MeasureY:   DISPATCH(QuantumMeasureGate,        QuantumOperator);
      case QO_MeasureX:   DISPATCH(QuantumMeasureGate,        QuantumOperator);
      default: assert(false && "Unknown quantum operator"); break;
      }
    }
...
#undef UNARYOP_FALLBACK
  ...
  // If the implementation doesn't implement quantum operator methods, fall back
  // on VisitQuantumOperator.
#define QUANTUMOP_FALLBACK(NAME) \
  RetTy VisitQuantum ## NAME(PTR(QuantumOperator) S, ParamTys... P) { \
    DISPATCH(QuantumOperator, QuantumOperator);    \
  }
  QUANTUMOP_FALLBACK(SingleQubitGate)
  QUANTUMOP_FALLBACK(MultiQubitGate) 
  QUANTUMOP_FALLBACK(MeasureGate)
  QUANTUMOP_FALLBACK(InitX)   QUANTUMOP_FALLBACK(InitZ)
  QUANTUMOP_FALLBACK(X)    QUANTUMOP_FALLBACK(Y)
  QUANTUMOP_FALLBACK(Z)    QUANTUMOP_FALLBACK(I)

  QUANTUMOP_FALLBACK(H)      QUANTUMOP_FALLBACK(Cnot)
  QUANTUMOP_FALLBACK(Swap)       QUANTUMOP_FALLBACK(Measure)
  QUANTUMOP_FALLBACK(MeasureZ)      QUANTUMOP_FALLBACK(MeausureX)
#undef QUANTUMOP_FALLBACK
