...
/// QCL QCC - gate : define the quantum-expression
/// Quantum Opeartor - This represents the quantum-expression.
///
class QuantumOperator final: public Expr {

  // QuantumOperator can has 2 values
  // e.g. x -> q1; (Val contains q1)
  //      cnot -> q1, q2; (Val contains q1, SubExprs contains q2)
  Stmt *SubExpr;
  
  Stmt **SubExprParams;
  unsigned NumExprs;
  
  // For multi-qubit gates: CNOT, SWAP
  static unsigned QubitID;

  public:
    typedef QuantumOperatorKind Opcode;
  
  protected:
    QuantumOperator(const ASTContext &Ctx, Expr *input, Opcode opc, QualType type, 
                    ExprValueKind VK, ExprObjectKind OK, SourceLocation l);
        
    QuantumOperator(const ASTContext &Ctx, Expr *inputFirst, ArrayRef<Expr *> params, Opcode opc, QualType type,
                    ExprValueKind VK, ExprObjectKind OK, SourceLocation l);

    QuantumOperator(const ASTContext &Ctx, Expr *inputFirst, ArrayRef<Stmt *> params, Opcode opc, QualType type,
                    ExprValueKind VK, ExprObjectKind OK, SourceLocation l);

    /// Build an empty quantum operator.
    explicit QuantumOperator(EmptyShell Empty): Expr(QuantumOperatorClass, Empty) {
      QuantumOperatorBits.Opc = QO_I;
    }
    
  public:
    
    static QuantumOperator *CreateEmpty(const ASTContext &C);

    /// Create Quantum Operator with one operand
    static QuantumOperator *Create(const ASTContext &C, Expr *input, Opcode opc,
                                   QualType type, ExprValueKind VK,
                                   ExprObjectKind OK, SourceLocation l);


    /// Create Quantum Operator with one operand and operands as parameters
    static QuantumOperator *Create(const ASTContext &C, Expr *inputFirst, ArrayRef<Expr *> params, 
                                         Opcode opc, QualType type, ExprValueKind VK,
                                         ExprObjectKind OK, SourceLocation l);

        /// Create Quantum Operator with one operand and operands as parameters
    static QuantumOperator *Create(const ASTContext &C, Expr *inputFirst, ArrayRef<Stmt *> params, 
                                         Opcode opc, QualType type, ExprValueKind VK,
                                         ExprObjectKind OK, SourceLocation l);

    /// Get Qubit ID for multi-qubit gate only
    static unsigned getQubitID() {return QubitID;};

    /// Set Qubit ID for multi-qubit gate only
    static void setQubitID(unsigned ID) {QubitID = ID;};

    /// Increase and descrease Qubit ID
    static void increaseQubitID(unsigned amount = 1) {QubitID += amount;};
    static void decreaseQubitID(unsigned amount = 1) {QubitID -= amount;};

    /// Get Operation Code
    Opcode getOpcode() const {
      return static_cast<Opcode>(QuantumOperatorBits.Opc);
    }
    void setOpcode(Opcode Opc)  {QuantumOperatorBits.Opc = Opc;}

    Expr* getSubExprVal() const {return cast<Expr>(SubExpr);}

    void setSubExprVal(Expr *E) { SubExpr = E; }

    /// getNumSubExprs - Return the size of the SubExprParams array.  This includes the
    /// constant expression, the actual arguments passed in, and the function
    /// pointers.
    unsigned getNumSubExprs() const { return NumExprs; }

    /// Retrieve the array of expressions.
    Expr **getSubExprs() { return reinterpret_cast<Expr **>(SubExprParams); }

    /// getExpr - Return the Expr at the specified index.
    Expr *getSubExpr(unsigned Index) const {
      assert((Index < NumExprs) && "Arg access out of range!");
      return cast<Expr>(SubExprParams[Index]);
    }
    const Expr *getExprC(unsigned Index) const {
      assert((Index < NumExprs) && "Arg access out of range!");
      return cast<Expr>(SubExprParams[Index]);
    }

    void setExprs(const ASTContext &Ctx, ArrayRef<Expr *> Exprs);

    /// Return the location of the operator.
    SourceLocation getOperatorLoc() const { return QuantumOperatorBits.Loc; }
    void setOperatorLoc(SourceLocation L) { QuantumOperatorBits.Loc = L; }


    /// Turn an Opcode enum value into the punctuation char it
    static StringRef getOpcodeStr(Opcode Op);

    /// Get the location at the begin of operator
    /// e.g: x -> q1;
    ///      ^
    SourceLocation getBeginLoc() const LLVM_READONLY {
    ...
    SourceLocation getEndLoc() const LLVM_READONLY {
      if (NumExprs > 0)
        return getExprC(NumExprs - 1)->getEndLoc();
      else
        return SubExpr->getEndLoc();
    }
    ...
    static bool classof(const Stmt *T){
      return T->getStmtClass() == QuantumOperatorClass;
    }

    // Iterators
    child_range children() { 
      if (NumExprs > 0){
        Stmt **tempArr = new Stmt*[NumExprs + 1 ];
        tempArr[0] = SubExpr;
        for (unsigned i = 0; i < NumExprs; i++){
          tempArr[i + 1] = SubExprParams[i];
        }
        return child_range(&tempArr[0], &tempArr[0] + NumExprs + 1);
      }
      else{
